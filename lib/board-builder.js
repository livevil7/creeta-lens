...
function loadOpenPRs(projectRoot) {
  ...
  if (cache && Date.now() - Date.parse(cache.fetchedAt) < PR_CACHE_TTL_MS) {
    return { ok: true, prs: cache.prs, fetchedAt: cache.fetchedAt, offline: false };
  }

  try {
    const out = execFileSync('gh',
      ['pr', 'list', '--json', 'number,title,headRefName,url', '--state', 'open'],
    ...
    try {
      ...
      fs.writeFileSync(cachePath, JSON.stringify({ fetchedAt, prs }, null, 2), 'utf-8');
    } catch { /* cache is an optimization, never a build blocker */ }
    return { ok: true, prs, fetchedAt, offline: false };
  } catch (err) {
    const reason = ghFailReason(err);
    // Expired cache + dead network: render what we last knew, dated.
    if (cache) return { ok: true, prs: cache.prs, fetchedAt: cache.fetchedAt, offline: true, reason };
...
function prNoteText(pr, reviewOn) {
  ...
  if (!reviewOn) return `PR ${pr.prs.length} open · board template lacks In Review anchors · 3 columns`;
  if (pr.offline) return `PR ${pr.prs.length} open · cached ${at} · ${pr.reason}`;
  return `PR ${pr.prs.length} open · checked ${at}`;
}
