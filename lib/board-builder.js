 ...
 * sits in. PR data comes from `gh pr list` (read-only), is cached in
 * .lens/pr-cache.json, and every failure path degrades to the plain 3 columns
 * with the reason printed on the board (never a silent "no PRs").
 *
...
// Returns { ok, prs, fetchedAt, offline, reason }. ok:false means "we don't know",
// which the board must say out loud — it is not the same as "no open PRs".
function loadOpenPRs(projectRoot) {
  ...
  try {
    ...
    if (c && Array.isArray(c.prs) && c.fetchedAt) cache = c;
  } catch { /* no/broken cache — refetch */ }

  if (cache && Date.now() - Date.parse(cache.fetchedAt) < PR_CACHE_TTL_MS) {
    return { ok: true, prs: cache.prs, fetchedAt: cache.fetchedAt, offline: false, truncated: !!cache.truncated };
  }

  try {
    const out = execFileSync('gh',
      ['pr', 'list', '--json', 'number,title,headRefName,url', '--state', 'open', '--limit', String(GH_PR_LIMIT)],
    ...
    try {
      ...
      fs.writeFileSync(cachePath, JSON.stringify({ fetchedAt, prs, truncated }, null, 2), 'utf-8');
    } catch { /* cache is an optimization, never a build blocker */ }
    return { ok: true, prs, fetchedAt, offline: false, truncated };
  } catch (err) {
    const reason = ghFailReason(err);
    // Expired cache + dead network: render what we last knew, dated.
    if (cache) return { ok: true, prs: cache.prs, fetchedAt: cache.fetchedAt, offline: true, reason, truncated: !!cache.truncated };
...
function prNoteText(pr, reviewOn) {
  ...
  if (!pr.ok) return `PR lookup unavailable — ${pr.reason} · 3 columns`;
  if (!reviewOn) return `PR ${pr.prs.length} open · board template lacks In Review anchors · 3 columns${truncNote}`;
  if (pr.offline) return `PR ${pr.prs.length} open · cached ${at} · ${pr.reason}${truncNote}`;
  return `PR ${pr.prs.length} open · checked ${at}${truncNote}`;
}
