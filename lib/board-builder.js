 ...
 * open PR *going to that doc's `base`* is a 4th column — the real (code-world)
 * state, not just the folder it sits in. PR data comes from `gh pr list`
 * (read-only), pinned with `--repo` to
...
function buildDoc(folder, base, dir) {
  ...
  if (hasMd) {
    ...
    baseRef = frontmatterValue(md, 'base');
...
function loadOpenPRs(projectRoot) {
  ...
  try {
    ...
    // Same reasoning for the shape: an entry written before the query asked for
    // baseRefName holds head names only, and every base check against it would
    // fail — emptying In Review for up to a TTL. Missing field = miss = refetch.
    // (An empty list passes trivially, and correctly: no PRs, nothing to join.)
    if (c && Array.isArray(c.prs) && c.fetchedAt && c.repo === target.id
        && c.prs.every(p => p && typeof p.baseRefName === 'string')) cache = c;
  ...
  try {
    const out = execFileSync('gh',
      ['pr', 'list', '--repo', target.id, '--json', 'number,title,headRefName,baseRefName,url', '--state', 'open', '--limit', String(GH_PR_LIMIT)],
      { cwd: projectRoot, timeout: GH_TIMEOUT_MS, encoding: 'utf-8', windowsHide: true,
...
//
// The head name alone does not qualify a PR. A stacked PR opened off the same
// branch onto some *other* base is a different piece of work: joining it puts
// the card under In Review, where the user reads "this task is being reviewed",
// while the thing actually in front of a reviewer is headed somewhere else. So
// when the doc records a `base`, the PR's baseRefName has to equal it.
//
// Docs with no `base` keep the name-only join. They are the large majority (this
// repo: 1 of 39 carries one), and every one of them predates the field, so
// requiring it would empty the column for the entire existing corpus — trading a
// narrow wrong join for a guaranteed missing one, and doing it invisibly. A doc
// that never stated its base is not evidence of a mismatch.
//
// Ties are possible on both paths — a fork PR can share head+base with an origin
// PR, and the name-only path can match a whole stack — and resolve to the lowest
// number: the first PR opened off that branch, i.e. the one the task itself
// opened. Stated explicitly because `gh`'s result ordering is not a contract and
// the board has to be byte-identical across rebuilds.
//
// Returns { count, baseMismatch }: baseMismatch is head matches rejected on base,
// which the top-bar note reports so this filter cannot fail silently either.
function splitReview(groups, prs) {
  const byBranch = new Map();
  for (const pr of prs) {
    if (!pr || !pr.headRefName) continue;
    const sameHead = byBranch.get(pr.headRefName);
    if (sameHead) sameHead.push(pr);
    else byBranch.set(pr.headRefName, [pr]);
  }
  const review = [];
  let baseMismatch = 0;
  groups.tasks = (groups.tasks || []).filter(d => {
    const heads = d.branch ? byBranch.get(d.branch) : null;
    if (!heads) return true;
    const qualified = d.base ? heads.filter(pr => pr.baseRefName === d.base) : heads;
    if (!qualified.length) { baseMismatch++; return true; }
    const pr = qualified.reduce((best, c) => (c.number < best.number ? c : best));
    review.push({ ...d, pr: { number: pr.number, url: pr.url, base: pr.baseRefName || null } });
    return false;
  });
  groups.review = review;
  return { count: review.length, baseMismatch };
}
...
function reviewPatches(eol) {
  ...
  // The base rides in the tooltip, not the badge text: which base a card was
  // qualified against is exactly what a reader needs to audit the join, but the
  // board is already 4 columns wide and "PR #2 → master" on the face of every
  // card spends horizontal space on a value that is the same for nearly all of
  // them. `el()` skips null attributes, so a legacy (base-less) join simply has
  // no tooltip rather than an empty one.
  const prBadge = '    if (doc.pr) meta.appendChild(el("a", { class: "badge pr", href: doc.pr.url, '
    + 'title: doc.pr.base ? ("PR #" + doc.pr.number + " \\u2192 " + doc.pr.base) : null, '
...
function prNoteText(pr, review) {
  ...
  if (!review) return `PR ${pr.prs.length} open · ${pr.repo} · board template lacks In Review anchors · 3 columns${truncNote}`;
  // A card held back because the only PR on its branch targets another base is a
  // deliberate non-join, but from the outside it looks identical to the feature
  // having quietly stopped working — so it is counted out loud, same as every
  // other "we are showing you less than you might expect" case on this bar.
  const skipNote = review.baseMismatch
    ? ` · ${review.baseMismatch} branch match${review.baseMismatch > 1 ? 'es' : ''} not joined (PR base ≠ task base)`
    : '';
  if (pr.offline) return `PR ${pr.prs.length} open · ${pr.repo} · cached ${at} · ${pr.reason}${truncNote}${skipNote}`;
  return `PR ${pr.prs.length} open · ${pr.repo} · checked ${at}${truncNote}${skipNote}`;
}
...
function main() {
  ...
  let review = null;   // null = In Review column not rendered (3-column board)
  if (pr.ok) {
    const patched = applyPatches(tpl, reviewPatches(eol));
    if (patched) {
      tpl = patched;
      review = splitReview(groups, pr.prs);
    }
  }
  const note = prNoteText(pr, review);
  ...
  const rendered = review === null ? FOLDERS : ['tasks', 'review', 'history', 'rules'];
