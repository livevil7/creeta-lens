...
function scanInstalledSkills() {
  ...
  for (const org of orgs) {
    ...
    for (const plugin of plugins) {
      ...
      // Use the latest semantic version. Lexical sorting picks 3.9.0 over
      // 3.10.0, so keep a small dependency-free comparator here.
      ...
      if (fs.existsSync(mcpJsonPath)) {
        ...
        if (pluginHasSkills) {
          // Hybrid plugin (Skill + MCP in same plugin): mark existing entries as MCP-enabled
...
function detectDomain(desc) {
  ...
  const domainMap = [
    [/error track|monitoring|logging|metric|tracing/i, 'Monitoring'],
...
function parseMcpFile(filePath, pluginName, versionPath) {
  try {
    ...
    // Unwrap {"mcpServers": {...}} wrapper format
...
function selectLatestVersion(versions) {
  return [...versions].sort(compareVersionNames).pop();
}

function compareVersionNames(a, b) {
  const parsedA = parseVersionName(a);
  const parsedB = parseVersionName(b);

  for (let i = 0; i < Math.max(parsedA.numbers.length, parsedB.numbers.length); i++) {
...
function parseVersionName(versionName) {
  const raw = String(versionName || '');
  const match = raw.match(/^v?(\d+(?:\.\d+)*)(?:[-+](.+))?$/);
  if (!match) return { raw, numbers: [-1], prerelease: raw };
