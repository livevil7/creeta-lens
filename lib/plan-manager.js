...
const { ensureLensDir, safeEnsureDir, safeReadJson, safeWriteJson } = require('./hook-utils');
...
function savePlanState(plan) {
  ...
  try {
    ensureLensDir(path.dirname(path.dirname(statePath)));
...
// v3.39 — what kind of plan this is (owner, 2026-09-14): "기획안이 처음 task 를
// 시작해서 목표를 세울 때가 있고, 기존 것을 변경·수정할 때가 있어. 기존 것을
// 개선할 때는 반드시 AS-IS → TO-BE 가 들어가야겠고". A research/comparison
// request is a third kind: it answers a question and never reaches /cc, so the
// execution floor (Inventory · Plan A · DoNotChange) does not apply to it.
// A document with no `kind:` is 신규 — legacy plans keep their current verdict.
const KIND_ALIASES = {
  '신규': ['신규', 'new', 'create', 'greenfield'],
  '개선': ['개선', '변경', '수정', 'improve', 'improvement', 'change', 'modify'],
  '조사보고': ['조사보고', '조사', '분석', 'report', 'research', 'analysis'],
};
const KIND_REQUIRED_SECTIONS = {
  '개선': ['AsIsToBe'],
};
const KIND_BASE_SECTIONS = {
  '조사보고': ['Goal', 'Evidence', 'Conclusion'],
};
...
function normalizeKind(value) {
  ...
  for (const [kind, aliases] of Object.entries(KIND_ALIASES)) {
    if (aliases.some(a => a.toLowerCase() === v)) return kind;
  }
  return null;
}

function readKind(content) {
  const fm = String(content || '').match(/^---\r?\n([\s\S]*?)\r?\n---/);
  if (!fm) return null;
  const m = fm[1].match(/^kind\s*:\s*(.+?)\s*$/m);
  return m ? normalizeKind(m[1]) : null;
}
...
function extractGoal(content) {
  ...
  // v3.39: skip 🚧 비목표 / Non-Goals — "비목표" contains 목표, and a plan that put
  // its non-goals first had them read as its success criteria.
  const headerRe = /^##\s+.*(?:🎯|Goal|목표|목적|目標|目标|Ziel|Objectif|Objetivo|Obiettivo)[^\n]*$/gm;
  let goalHeader = null;
  for (const m of content.matchAll(headerRe)) {
    if (!/비목표|non[-\s]?goal|🚧/i.test(m[0])) { goalHeader = m; break; }
  }
  ...
  for (const line of raw.split('\n')) {
    ...
    // v3.39: top-level items only. A nested bullet is a detail of the goal above
    // it, not another goal — counting it inflated "성공기준 N" on the approval screen.
    const checkbox = line.match(/^ {0,1}(?:[-*+]|\d+[.)])\s*\[[ xX]\]\s*(.+?)\s*$/);
    if (checkbox) { checkboxCriteria.push(checkbox[1]); continue; }
    // Numbered goals ("1. 회계팀이 …") are how most hand-written plans list them —
    // 47 of the workspace's 105 plans had goals the parser could not see.
    const bullet = line.match(/^ {0,1}(?:[-*+]|\d+[.)])\s+(.+?)\s*$/);
    if (bullet) plainBullets.push(bullet[1]);
  }
  // A goal table (`| G1 | 목표 | 통과선 |`) — take the goal column, or the first
  // column that is not an id.
  if (!checkboxCriteria.length && !plainBullets.length) {
    const tableLines = raw.split('\n').filter(l => l.trim().startsWith('|'));
    const hIdx = tableLines.findIndex((l, i) => !isSeparatorRow(l) && isSeparatorRow(tableLines[i + 1] || ''));
    if (hIdx !== -1) {
      const header = splitRow(tableLines[hIdx]);
      let gc = header.findIndex(h => /목표|goal|what|가능해지는/i.test(h));
      if (gc === -1) gc = header.findIndex(h => !/^(?:#|id|no\.?|번호)?$/i.test(h.replace(/\*/g, '').trim()));
      for (const l of tableLines.slice(hIdx + 2)) {
        const cell = (splitRow(l)[gc] || '').replace(/\*\*/g, '').trim();
        if (cell && !EMPTY_CELL.test(cell)) plainBullets.push(cell);
      }
    }
  }
...
const SECTION_ALIASES = {
  ...
  // v3.39 — kind-specific sections (see KIND_REQUIRED_SECTIONS)
  'AsIsToBe': ['AS-IS', 'AS IS', 'As-Is', 'ASIS', 'TO-BE', 'Before/After', 'Before → After', '현재 → 변경', '현재→변경', '현행 → 개선', '전후 비교'],
  'Evidence': ['📊', 'Evidence', 'Findings', '근거', '조사 결과', '조사결과'],
  'Conclusion': ['💡', 'Conclusion', 'Recommendation', '결론', '권고'],
...
// v3.39: `in` removed — status cells were matched by substring, so "보류 (pending)"
// read as included through the `in` of "pending", and "포함 안 함" read as included.
const INCLUDED_MARKERS = ['포함', 'included', 'include', '含む', '包含'];
const DEFERRED_MARKERS = ['보류', 'deferred', 'pending', 'hold', '保留', '暂缓'];

/**
 * Classify a ledger status cell by its FIRST word, not by substring.
 *
 * Leading symbols (✅, ⏸) are skipped. A negation right after 포함 ("포함 안 함")
 * is not an inclusion — it is an unreadable status and is reported as one.
 *
 * @returns {{kind: 'included'|'excluded'|'deferred'|'unknown', rest: string}}
 */
function classifyStatus(status) {
  const s = String(status || '').replace(/\*/g, '').replace(/^[^\p{L}]+/u, '').trim();
  const m = s.match(/^[^\s:：\-–—(（,，]+/);
  const head = (m ? m[0] : '').toLowerCase();
  const rest = s.slice(m ? m[0].length : 0).trim();
  const is = list => list.some(x => x.toLowerCase() === head);
  if (is(EXCLUDED_MARKERS)) return { kind: 'excluded', rest };
  if (is(DEFERRED_MARKERS)) return { kind: 'deferred', rest };
  if (is(INCLUDED_MARKERS)) {
    if (/^(?:안|않|아님|미\b|x\b|no\b|not\b)/i.test(rest)) return { kind: 'unknown', rest };
    return { kind: 'included', rest };
  }
  return { kind: 'unknown', rest };
}

const reasonOf = rest => String(rest || '').replace(/^[\s:：\-–—]+/, '').trim();
...
function splitRow(line) {
  ...
  // v3.39: a `|` inside a code span is content, not a column break — plans put
  // grep patterns and `a || b` in cells, and the old split pushed the status into
  // the wrong column ("상태를 해석할 수 없다" on a correct row).
  const cells = [];
  let cur = '';
  let tick = 0;
  ...
  // An unclosed backtick would swallow the rest of the row — fall back.
...
function readInventoryTable(content) {
  ...
  // A stray raw `|` in the item text makes the row one cell too long. When the
  // item column comes before the others, read those from the right edge and give
  // the overflow back to the item — the status is always the last word of a row.
  const itemFirst = col.item < Math.min(col.source, col.where, col.status);
  const clean = v => (v || '').replace(/\*/g, '').trim();
  const toRow = r => {
    const extra = r.length - width;
    if (extra <= 0 || !itemFirst) {
      return { item: clean(r[col.item]), source: clean(r[col.source]), where: clean(r[col.where]), status: clean(r[col.status]) };
    }
    const at = idx => clean(r[idx > col.item ? idx + extra : idx]);
    return {
      item: clean(r.slice(col.item, col.item + extra + 1).join(' | ')),
...
function validatePlanCoverage(rawContent) {
  const result = { valid: false, total: 0, included: 0, excluded: 0, deferred: 0, problems: [] };
  ...
  for (const row of rows) {
    ...
    const { kind, rest } = classifyStatus(status);

    if (kind === 'excluded' || kind === 'deferred') {
      result[kind] += 1;
      // "제외" alone is not an answer. Require prose past the marker.
      if (reasonOf(rest).length < 2) {
        const word = kind === 'excluded' ? '제외' : '보류';
        result.problems.push(`${label}: ${word} 사유가 없다 — "${word}: 왜 이번 범위 밖인가" 형태로 적는다`);
      }
      ...
      result.problems.push(`${label}: 상태 "${status}" 를 해석할 수 없다 — "포함" · "제외: 사유" · "보류: 사유" 중 하나로 시작한다`);
    }
  }
...
// v3.39: a plan is free to name its method section in plain words. When there is
// no "Plan A" heading, the How block is read instead (with any Plan B cut off).
const HOW_ALIASES = ['🛠', 'How', '어떻게', '방법', '실행 계획', '구현 계획'];
const STEPS_ALIASES = ['단계', 'Steps', 'Step', 'ステップ', '步骤', 'Pasos', 'Étapes', 'Etapes', 'Schritte', 'Passi', '절차', 'Tasks'];

const normalizeWork = t => String(t || '')
  .toLowerCase()
...
function deriveTodoItems(rawContent) {
  // v3.39: two layers. `problems` is what a person reads ("무엇이 없고 무엇을 적나");
  // `hints` names the /cp phase for the model. The old single layer said "Phase 1
  // 회귀", and the model repeated it to the owner as "없다" every time.
  const result = { valid: false, total: 0, goals: [], inventory: [], steps: [], problems: [], warnings: [], hints: [] };
  const content = String(rawContent).replace(/\r\n/g, '\n');
  ...
  if (!goal || !goal.successCriteria.length) {
    result.problems.push('목표(🎯) 에서 성공 기준을 읽을 수 없다 — "이게 끝나면 무엇이 가능해지나" 를 불릿으로 적는다');
    result.hints.push('Phase 0 (목표)');
  } else {
    result.goals = goal.successCriteria.filter(t => !isTemplateLeftover(t));
    if (!result.goals.length) {
      result.problems.push('목표가 템플릿 자리표시자(`{…}`)뿐이다 — 사람 말 목표로 채운다');
      result.hints.push('Phase 0 (목표)');
    }
  }
  ...
  const { problem, rows } = readInventoryTable(content);
  if (problem) {
    result.problems.push(`${problem} — 인벤토리 없이는 실행 목록을 만들 수 없다`);
    result.hints.push('Phase 2.45 (작업 인벤토리)');
  } else {
    for (const row of rows) {
      if (classifyStatus(row.status).kind === 'included') result.inventory.push(row.item);
    }
    ...
    if (!result.inventory.length) {
      result.problems.push(rows.length
        ...
        : '작업 인벤토리에 실행할 항목이 없다 (빈 표 또는 `{자리표시자}` 뿐)');
      result.hints.push('Phase 2.45 (작업 인벤토리)');
    }
  }

  // 3. Steps. sliceSection stops at the first heading of any level and Plan A's
  //    steps live in a child heading, so this needs the level-relative slicer.
  //    v3.39: steps are optional. A plan whose method is written as prose is still
  //    executable from its goals and inventory — refusing it sent every plan that
  //    did not copy the template's `#### 단계` back to "Phase 1" (owner: "update
  //    todo 하면 뭐가 계속 없다고만 찡찡거린다").
  let block = sliceByHeading(content, (text, lvl) => lvl >= 2 && headingStartsWith(text, PLAN_A_ALIASES));
  if (block === null) {
    block = sliceByHeading(content, (text, lvl) => lvl >= 2 && headingStartsWith(text, HOW_ALIASES));
    if (block !== null) {
      const cut = block.search(/^ {0,3}#{1,6}\s+Plan B/m);
      if (cut !== -1) block = block.slice(0, cut);
    }
  }
  if (block === null) {
    result.warnings.push('어떻게(🛠 / Plan A) 섹션을 찾지 못했다 — 목표와 인벤토리로만 실행 목록을 만든다');
  } else {
    ...
    // execution steps and still returned valid:true. Unmarked prose is not steps.
    const seen = new Set(result.inventory.map(normalizeWork));
    result.steps = stepsInPlanA(block).filter(t => !seen.has(normalizeWork(t)));
    if (!result.steps.length) {
      result.warnings.push('어떻게(🛠) 에 체크박스 단계가 없다 — 목표와 인벤토리로만 실행 목록을 만든다');
    }
  }
 ...
 */
function validatePlanStructure(content, grade, kind) {
  const missing = [];
  const warnings = [];
  const k = normalizeKind(kind) || readKind(content) || '신규';
  const base = KIND_BASE_SECTIONS[k] || REQUIRED_SECTIONS;
  const required = base
    .concat(KIND_BASE_SECTIONS[k] ? [] : (GRADE_REQUIRED_SECTIONS[grade] || []))
    .concat(KIND_REQUIRED_SECTIONS[k] || []);
  ...
  // Leftover template placeholders. v3.39: only a list item, table cell, heading
  // or line that is NOTHING BUT `{…}` is an unfilled template — that is what the
  // template leaves behind. A `{id}` inside a sentence, a code span or a fence is
  // how plans name things (7 in-flight plans were refused for `{job_uuid}`), so
  // it is a warning at most. Shell/git syntax (%{…}, ${…}, @{…}) never counts.
  let hard = false;
  let soft = false;
  let fence = null;
  for (const raw of stripComments(content).split('\n')) {
    const f = FENCE.exec(raw);
    if (f) {
      if (fence === null) { if (opensFence(f)) { fence = f[1]; continue; } } else if (isCloser(f, fence)) { fence = null; continue; }
    }
    if (fence !== null) continue;
    const t = raw.trim();
    if (!t) continue;
    if (t.startsWith('|')) {
      if (splitRow(t).some(c => c && isTemplateLeftover(c))) hard = true;
      continue;
    }
    const body = t
      .replace(/^#{1,6}\s+/, '')
      .replace(/^>\s*/, '')
      .replace(/^(?:[-*+]|\d+[.)])\s+(?:\[[ xX]\]\s*)?/, '');
    if (body && isTemplateLeftover(body)) { hard = true; continue; }
    if (/(?<![%$@])\{[a-z_]+\}/i.test(body.replace(/`[^`]*`/g, ''))) soft = true;
  }
  if (hard) {
    missing.push('(contains unresolved {placeholder} variables)');
  } else if (soft) {
    warnings.push('본문에 {중괄호} 표기가 있다 — 템플릿 잔재가 아니면 그대로 둬도 된다');
  }
  return { valid: missing.length === 0, missing, warnings, kind: k };
}
