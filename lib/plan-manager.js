 ...
 * @param {Array} [planData.goal.verification] - [{criterion, method, expected}] — how each success criterion is verified (evidence must surface in transcript)
...
function generatePlanContent(planData) {
  ...
  // === Verification (how each success criterion is confirmed) ===
  content += `## ✅ ${h.verification}\n\n`;
  content += `| # | ${h.vCriterion} | ${h.vMethod} | ${h.vExpected} |\n`;
  content += `|---|---|---|---|\n`;
  const verification = goal.verification || [];
  if (verification.length > 0) {
    verification.forEach((v, i) => {
      content += `| ${i + 1} | ${v.criterion || '-'} | ${v.method || '-'} | ${v.expected || '-'} |\n`;
    });
  } else {
