 ...
 *   `GRADE_EXEMPT_SECTIONS` — now called for real by the /cp Phase 5.0 gate and
...
// Grade-specific relaxations. `fast` plans intentionally skip the heavier
// sections (see the speed-grade table in /cp SKILL.md); enforcing them there
// would turn small fixes into ceremony.
const GRADE_EXEMPT_SECTIONS = {
  fast: ['Alternatives', 'Risks'],
  standard: [],
  deep: [],
};
 ...
 * @param {string} [grade] - 'fast' | 'standard' | 'deep' (default: 'standard')
 * @returns {{valid: boolean, missing: string[]}}
 */
function validatePlanStructure(content, grade) {
  const missing = [];
  const exempt = GRADE_EXEMPT_SECTIONS[grade] || [];
