...
function completeAgentByHarnessId(agentId, status, { agentType = null, fromWorkflow = false } = {}) {
  const id = harnessId(agentId);
  if (!id) return null;
  return mutateDashboard((dashboard) => {
    const agents = dashboard.agents;
    const open = a => a.status === LAUNCHED_STATUS || OPEN_STATUSES.has(a.status);
    const known = agents.find(a => harnessId(a.agentId) === id);
    ...
    if (agent) {
      // Remember whose stop this was, so the same id stopping again (resumed)
      // is recognised above instead of resolving yet another entry.
      agent.agentId = id;
      return finishAgent(dashboard, agent, status || 'done', null);
    }
    // No entry to resolve yet: the stop may have beaten this agent's PostToolUse
    // (still `running`, no id). Park it; markAgentLaunched() consumes it.
    dashboard.earlyStops = [...(dashboard.earlyStops || []), { agentId: id, at: new Date().toISOString() }]
      .slice(-MAX_EARLY_STOPS);
    return null;
  });
}

/** Harness agent ids appear as `agent-abc123` (SubagentStart) and `abc123` (envelope, SubagentStop). */
function harnessId(id) {
  return String(id || '').trim().replace(/^agent-/, '');
}
...
function markAgentLaunched({ toolUseId = null, description = '', agentId = null, runId = null, name = null } = {}) {
  return mutateDashboard((dashboard) => {
    ...
    const id = harnessId(agentId);
    if (id) agent.agentId = id;
    if (runId) agent.runId = runId;
    if (name) agent.name = extractAgentName(name);
    // Its SubagentStop already arrived (completeAgentByHarnessId parked it): done.
    const early = id ? (dashboard.earlyStops || []).findIndex(s => s && harnessId(s.agentId) === id) : -1;
    if (early >= 0) {
      dashboard.earlyStops.splice(early, 1);
      return finishAgent(dashboard, agent, 'done', null);
    }
