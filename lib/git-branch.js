 ...
 * @param {{requireTaskBranch?: boolean}} [opts]
...
function entryDecision(repoPath, planBranch, opts) {
  const { requireTaskBranch = false } = opts || {};
  ...
  if (pre.dirty) demote('ask', 'working tree dirty — 무관한 변경이 task 브랜치를 따라간다');
  ...
  for (const i of pre.issues) if (!reasons.includes(i)) reasons.push(i);
