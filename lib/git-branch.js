 ...
 * @returns {boolean}
 */
// 통합 브랜치가 아닌 것으로 규칙(§3)이 이미 열거한 접두사들. 허용 4종은 지금
// 쓰는 이름이고, 이쪽은 "쓰지 않는다" 로 금지됐지만 과거에 만들어져 남아 있는
// 이름들이다. 실측: livevil-research 가 열린 PR 의 head(claude/…)에 체크아웃돼
// 있어 그 브랜치가 base 로 판정됐다 — /cp 가 그대로 계획하면 PR 이 main 이 아니라
// 그 PR 브랜치를 향한다. 금지 접두사도 base 후보에서 뺀다.
const NON_INTEGRATION_PREFIXES = ['sync', 'agent', 'claude', 'codex', 'task', 'feature', 'backup'];

function isTaskShapedBranch(name) {
  const prefixes = getBranchPrefixes().concat(NON_INTEGRATION_PREFIXES);
...
/**
 * Read one frontmatter scalar from a plan document (quotes stripped, CRLF-safe).
 * Missing file / field / `null` → null.
 * @param {string|undefined} planDoc
 * @param {string} key
 * @returns {string|null}
 */
function planFrontmatterValue(planDoc, key) {
  if (!planDoc) return null;
  let content;
  try { content = fs.readFileSync(planDoc, 'utf-8'); } catch { return null; }
...
/**
 * Evidence that a branch with no remaining ref reached origin/<base>
 * (judgeRemoteMissing 전용):
 *   ① the plan's frontmatter `last_tip:` (the /cc Phase 7 records it) is an
 *      ancestor of origin/<base> — covers fast-forward merges too. When a tip
 *      is recorded it is the only evidence consulted: it names this task's
 *      commit, a merge message only names a branch.
 *   ② otherwise a merge commit on origin/<base> whose subject names the branch
 *      as a whole token — `Merge branch 'fix/foo'`, `Merge pull request #1 from
 *      o/fix/foo`. `fix/foo-v2` must not prove `fix/foo` (nor `fix/foo` prove
 *      `foo`), and a same-named
 *      merge from before the plan's `created:` date is an earlier task's (names
 *      come from the slug — see verifyOwnership).
 * @returns {{reason: string, baseSha: string}|null}
 */
function deletedBranchEvidence(repoPath, branch, base, planDoc) {
  const baseRef = `origin/${base}`;
  const baseSha = git(repoPath, ['rev-parse', '--verify', '--quiet', `refs/remotes/${baseRef}`]);
  if (!baseSha) return null;

  const tip = planFrontmatterValue(planDoc, 'last_tip');
  if (tip) {
    if (/^[0-9a-f]{7,40}$/i.test(tip)
        && gitExitCode(repoPath, ['merge-base', '--is-ancestor', tip, baseRef]) === 0) {
      return { baseSha, reason: `계획서 last_tip ${tip.slice(0, 12)} 이 ${baseRef} 의 조상 — 기록된 마지막 커밋이 base 에 들어갔다` };
    }
    return null;
  }

  const created = planFrontmatterValue(planDoc, 'created');
  // a bare date keeps the current time-of-day in git's approxidate — pin midnight
  const since = created && /^\d{4}-\d{2}-\d{2}/.test(created) ? [`--since=${created.slice(0, 10)} 00:00:00`] : [];
  const log = git(repoPath, ['log', baseRef, '--merges', ...since, '-F', `--grep=${branch}`, '--format=%H %s']);
  if (!log) return null;
  const esc = branch.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  // A `/` before the name is only a boundary right after the PR owner — otherwise
  // `Merge branch 'fix/foo'` would prove a branch named `foo`.
  const token = new RegExp(
    `(^|['\\s])${esc}(['\\s]|$)`            // Merge branch 'fix/foo' [into …]
    + `|remote-tracking branch '[^'\\s/]+/${esc}'` // … 'origin/fix/foo' (remote name has no `/`)
    + `|from\\s+[^\\s/]+/${esc}(\\s|$)`);      // Merge pull request #7 from owner/fix/foo
  const hit = log.split('\n').find(l => token.test(l.slice(l.indexOf(' ') + 1)));
  if (!hit) return null;
  return { baseSha, reason: `${baseRef} 이력에 이 브랜치의 병합 커밋 ${hit.slice(0, 12)} ("${hit.slice(hit.indexOf(' ') + 1)}") — 병합 뒤 브랜치를 지운 흐름` };
}
 ...
 * PR 병합 사실을 명시적으로 주입한 경우(opts.prMerged === true), 또는
 * origin/<base> 이력에 이 브랜치의 병합 커밋이 있거나 계획서(opts.planDoc)
 * 의 last_tip 이 base 조상일 때(deletedBranchEvidence)만 'merged-deleted'
 * 로 판정한다 — 이때는 지울 ref 자체가 없어 오판이 ref 삭제로 이어질 수
 * 없고, 주입값의 진위 책임은 gh 로 조회한 호출자에 있다.
 *
 * 'unpushed' / 'unknown' 의 경계는 **증명의 방향**이다. 두 상태는 /cp done
 * 의 처리가 다르므로(unpushed → push+PR 제안, unknown → 자동 처리 금지·사람
 * 확인) 섞으면 안 된다:
 *   · 'unpushed' = "원격에 없음" + "내용이 base 에 **없음이 확인됨**"
 *     (merge-tree 가 실제로 돌았고 결과 tree 가 base tree 와 다름) —
 *     positive 한 증명이 있을 때만.
 *   · 'unknown'  = 증명 자체가 불가능/실패 (merge-tree 충돌·구버전 git<2.38,
 *     계보 다름, 비교 기준 부재). 거짓 "머지됨"을 금지하는 것과 같은 이유로
 *     거짓 "미푸시"도 만들지 않는다.
 *
 * upstream 설정(branch.<branch>.merge)은 fetch --prune 후에도 남는 "과거
 * push 흔적"이다 — reason 을 풍부하게 하는 데만 쓰고 **판정 근거로는 쓰지
 * 않는다**. 있다고 해서 머지를 단정할 수 없고, 없다고 해서 미푸시를 단정할
 * 수도 없다: `git push origin <branch>` (-u 없음) 는 흔적을 남기지 않으므로
 * "흔적 없음 + 증명 실패"는 -u 없이 push 후 원격에서 삭제된 브랜치와 구분
 * 불가능하다.
 *
 * @param {string} repoPath
 * @param {string} branch
 * @param {string} base
 * @param {{prMerged?: boolean, planDoc?: string}} [opts]
...
function judgeRemoteMissing(repoPath, branch, base, opts) {
  ...
  if (localSha === null || localSha === '') {
    ...
    // I1 — PR 없이 `merge --no-ff` 로 합치고 브랜치를 지우는 레포(Returns ERP)는
    // 6개 중 4개가 여기서 unknown 이었다. ref 는 없어도 base 이력에 남은 증거
    // 두 가지는 읽을 수 있다. 지울 ref 가 없으니 오판이 ref 삭제로 이어지지 않는다.
    const evidence = deletedBranchEvidence(repoPath, branch, base, opts && opts.planDoc);
    if (evidence) {
      return { state: 'merged-deleted', unmergedPatches: 0, branchSha: null, baseSha: evidence.baseSha, localSha: null, reason: `origin/${branch}·refs/heads/${branch} 모두 부재 — ${evidence.reason}` };
    }
    // 원격도 로컬도 없다 = 비교할 내용 자체가 없다. positive 증명이 0 이므로
    // 'unpushed'(= "내용이 base 에 없음이 증명됨") 를 주장할 수 없다. `-u` 없이
    // push → 머지 → 로컬까지 삭제된 브랜치가 정확히 여기 떨어진다.
    return { state: 'unknown', unmergedPatches: null, branchSha: null, baseSha: null, localSha: null, reason: `origin/${branch} 도 refs/heads/${branch} 도 없음 — 비교할 ref 가 없어 내용 반영 여부를 증명할 수 없다. 미푸시 단정도 병합 단정도 금지, 사람 확인 필요 (호출자가 PR 병합 상태를 안다면 opts.prMerged 로 주입)` };
  }
  ...
  // upstream 설정(branch.<branch>.merge)은 **판정 근거가 아니라** reason 을
  // 풍부하게 하는 재료다: `git push origin <branch>` (-u 없음) 는 흔적을
  // 남기지 않으므로 "흔적 없음"이 "push 된 적 없음"을 뜻하지 않는다.
  const upstreamTrace = git(repoPath, ['config', '--get', `branch.${branch}.merge`]);
  if (mergedTree === null || baseTree === null) {
    // merge-tree 실패(충돌·구버전 git<2.38) — 내용이 base 에 있는지 **없는지**
    // 조차 증명 불가 → upstream 흔적 유무와 무관하게 'unknown'.
    // (거짓 "미푸시" 금지: -u 없이 push 한 뒤 원격에서 삭제된 브랜치가 정확히
    //  이 모양 — upstream 흔적 없음 + 내용 증명 실패. 여기서 'unpushed' 를
    //  반환하면 /cp done 이 이미 머지됐을 수도 있는 작업에 push+PR 재생성을
    //  제안한다. 'unknown' 은 자동 처리 금지·사람 확인 경로다.)
    const proofNote = upstreamTrace
      ? `upstream 설정(branch.${branch}.merge) 흔적 있음 — 과거 push 이력 존재`
      : 'upstream 설정 흔적 없음 — 단, -u 없는 push 는 흔적을 남기지 않으므로 이것이 미푸시의 증거는 아님';
    return { state: 'unknown', unmergedPatches: null, branchSha: null, baseSha, localSha, reason: `origin/${branch} 없음 + merge-tree 검사 실패(충돌·구버전 git)로 로컬 tip 내용이 origin/${base} 에 반영됐는지 증명 불가 — 병합 단정도 미푸시 단정도 금지, 사람 확인 필요. ${proofNote}` };
  }

  // 여기부터는 positive 증명 있음: merge-tree 가 실제로 돌았고 결과 tree 가
  // origin/<base> tree 와 다르다 = 로컬 tip 에 base 미반영 내용이 **있음이
  // 확인됐다**. 'unpushed' 는 이 증명이 있을 때만 반환한다.
 ...
 *         명시적으로 주입한 경우(opts.prMerged === true), 또는 base 이력의
 *         병합 커밋·계획서 last_tip(opts.planDoc) 증거가 있을 때만
 *         'merged-deleted', 아니면 'unknown'
 *      c. 로컬 tip 에 base 미반영 내용이 **있음이 확인되면**(merge-tree 가
 *         돌았고 tree 가 base 와 다름) 'unpushed' (upstream 설정 흔적이
 *         있으면 "병합 없이 원격에서 삭제됐을 수 있음" 경고 포함).
 *         증명 수단 실패(merge-tree 충돌·구버전 git·계보 다름)는 병합도
 *         미푸시도 단정하지 않고 'unknown' — upstream 흔적 유무는 판정을
 *         바꾸지 않는다(-u 없는 push 는 흔적을 남기지 않으므로 흔적 부재가
 *         미푸시의 증거가 못 된다). 거짓 "머지됨" 금지와 같은 이유로 거짓
 *         "미푸시"도 금지 — 둘은 /cp done 의 처리가 다르다
 ...
 * @param {{fetch?: boolean, prMerged?: boolean, planDoc?: string}} [opts] - fetch: true 면 판정
 ...
 *   planDoc: 계획서 경로 — ref 가 모두 없을 때 frontmatter `last_tip:` 을 증거로 읽는다
...
function preflight(repoPath) {
  ...
  // status 를 읽지 못하면(인덱스·워크트리 손상, 권한 등) dirty 는 false 가 되지만
  // 그건 "깨끗하다" 가 아니라 "모른다" 다. 그대로 두면 /cc 가 알 수 없는 작업트리를
  // clean 으로 취급해 브랜치 생성으로 진행한다 — dirty fail-closed 규칙이 무력해진다.
  const statusUnknown = status === null;
  if (statusUnknown) issues.push('git status --porcelain 실패 — 작업트리 상태를 읽을 수 없다(인덱스·워크트리 손상 등). clean 으로 간주하지 않는다');
  ...
  return {
    ok: !diverged && !divergenceUnknown && !statusUnknown && base !== null,
    dirty,
    diverged,
    base,
    issues,
...
/**
 * Decide whether /cc may enter execution here, and on which branch.
 *
 * Why this exists (v3.34). `preflight()` returns raw facts — {ok, dirty,
 * diverged, base, issues} — and every consumer re-derived the *decision* from
 * them in prose. The audit found the ownership proof (0.4 5-a) had 57 lines of
 * SKILL.md and zero lines of code, while four of five research lenses assumed
 * the judgement already lived here. It did not. A rule stated only in prose is
 * one the model can restate wrongly, and this rule guards the path that once
 * committed straight to a repo whose base is `staging` — where merging deploys
 * (see this file's header).
 *
 * Deliberately conservative: anything it cannot prove becomes `ask`, never
 * `proceed`. `block` is reserved for states where continuing would put commits
 * somewhere they cannot be taken back from.
 *
 * Dirty excludes the plan document being executed (`opts.planDoc`, absolute
 * or repo-relative) and `.lens/` runtime state — /cc 진입 7회가 전부 계획서
 * 파일 하나 때문에 ask 였다(G10). Anything else still makes it `ask`.
 *
 * @param {string} repoPath
 * @param {string|null} planBranch - branch named by the plan document, if any
 * @param {{requireTaskBranch?: boolean, planDoc?: string}} [opts]
 * @returns {{decision:'proceed'|'ask'|'block', reasons:string[], base:string|null,
 *            branch:string|null, onPlanBranch:boolean, preflight:object}}
 */
function entryDecision(repoPath, planBranch, opts) {
  const { requireTaskBranch = false, planDoc = null } = opts || {};
  const pre = preflight(repoPath);
  const current = git(repoPath, ['rev-parse', '--abbrev-ref', 'HEAD']);
  const onPlanBranch = Boolean(planBranch) && current === planBranch;
  const reasons = [];
  let decision = 'proceed';

  // block outranks ask — a later 'ask' must never soften an earlier 'block'.
  const demote = (level, why) => {
    reasons.push(why);
    decision = (level === 'block' || decision === 'block') ? 'block' : 'ask';
  };

  // Without a base we cannot prove what a commit would sit on, and canCommitTo
  // cannot rule out the base branch itself.
  if (pre.base === null) demote('ask', 'base 판정 불가 — 추정으로 진행하지 않는다');
  if (pre.diverged) demote('block', '원격과 양방향 분기 — 자동 진행 시 이력이 꼬인다. 수동 해결 필요');
  if (pre.dirty) {
    const stray = strayChanges(repoPath, planDoc);
    if (stray === null) {
      demote('ask', 'working tree dirty — 무관한 변경이 task 브랜치를 따라간다');
    } else if (stray.length > 0) {
      const shown = stray.slice(0, 5).join(', ') + (stray.length > 5 ? ` 외 ${stray.length - 5}개` : '');
      demote('ask', `working tree dirty — 무관한 변경이 task 브랜치를 따라간다 (${shown})`);
    }
  }

  if (current === null) {
    demote('block', '현재 브랜치를 읽을 수 없다');
  } else if (pre.base !== null && current === pre.base) {
    demote(requireTaskBranch ? 'block' : 'ask',
      'base(' + pre.base + ') 에 체크아웃된 상태 — task 브랜치 없이 실행하면 커밋이 base 로 간다');
  }
  if (planBranch && current !== null && current !== planBranch) {
    demote('ask', '현재 브랜치(' + current + ') 가 계획 브랜치(' + planBranch + ') 와 다르다');
  }

  // preflight 의 dirty 사유는 위의 판정(계획서·.lens 제외)이 대신한다 — 그대로
  // 실으면 같은 사유가 문구만 바꿔 두 번 나오고, 제외된 파일만 있을 때도 남는다.
  for (const i of pre.issues) {
    if (i.startsWith('working tree dirty')) continue;
    if (!reasons.includes(i)) reasons.push(i);
  }
  return { decision, reasons, base: pre.base, branch: planBranch || null, onPlanBranch, preflight: pre };
}

/**
 * Changed paths (repo-relative) other than the plan document and `.lens/`.
 * `-z --untracked-files=all` so a new plan in a new folder is listed as itself
 * (not `?? docs/`) and non-ASCII names are not C-quoted. null = unreadable.
 * @param {string} repoPath
 * @param {string|null} planDoc
 * @returns {string[]|null}
 */
function strayChanges(repoPath, planDoc) {
  let out;
  try {
    // not git(): its trim() would eat the leading space of the first ` M path` entry
    out = execFileSync('git', ['-C', repoPath, 'status', '--porcelain', '-z', '--untracked-files=all'], {
      encoding: 'utf-8', stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true,
    });
  } catch {
    return null;
  }
  // show-toplevel is a resolved path (macOS /var → /private/var) — resolve the
  // plan the same way or the comparison misses on symlinked temp/home dirs.
  const real = p => { try { return fs.realpathSync(p); } catch { return p; } };
  const top = git(repoPath, ['rev-parse', '--show-toplevel']) || repoPath;
  const plan = planDoc
    ? path.relative(real(top), real(path.resolve(repoPath, planDoc))).split(path.sep).join('/')
    : null;
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    if (entry[0] === 'R' || entry[0] === 'C') i++; // -z puts the rename source in the next field
  }
  return paths.filter(p => p !== plan && p !== '.lens' && !p.startsWith('.lens/'));
}

/**
 * Prove a branch belongs to this plan rather than merely sharing its name.
 *
 * Branch names come deterministically from the slug, so a previous task's
 * leftover branch and a brand-new task produce the same name. Checking out on a
 * name match alone stacks new work on unrelated history and breaks
 * `1 task = 1 branch`. The name says *which* branch, never *whose*.
 *
 * Three independent signals, all required:
 *   1. the plan document carries an ownership record naming this branch;
 *   2. the branch actually forks from the resolved base (shared merge-base);
 *   3. it is not already merged — a finished branch is not a resume.
 *
 * @param {string} repoPath
 * @param {string} branch
 * @param {string} base
 * @param {string} planContent - full text of the plan document
 * @returns {{owned:boolean, checks:{record:boolean, forkedFromBase:boolean, notMerged:boolean},
 *            reasons:string[]}}
 */
function verifyOwnership(repoPath, branch, base, planContent) {
  const reasons = [];
  const LF = String.fromCharCode(10);

  // Substring match, not a built regex: branch names carry `/` and `.`, and
  // escaping them into a pattern is a bug surface that buys nothing here.
  const OWNERSHIP_MARKER = '**작업 브랜치**';
  const recordLine = String(planContent || '')
    .split(LF)
    .find(l => l.indexOf(OWNERSHIP_MARKER) !== -1);
  const record = Boolean(recordLine && recordLine.indexOf(branch) !== -1);
  if (!record) {
    reasons.push('계획 문서에 ' + branch + ' 소유 기록이 없다 — 이름이 같을 뿐 이 계획이 만든 브랜치라는 증거가 없다');
  }

  const mb = git(repoPath, ['merge-base', branch, 'origin/' + base]);
  const forkedFromBase = mb !== null && mb !== '';
  if (!forkedFromBase) {
    reasons.push(branch + ' 와 origin/' + base + ' 의 merge-base 를 찾을 수 없다 — 무관한 이력일 수 있다');
  }

  let notMerged = true;
  try {
    const st = mergedState(repoPath, branch, base, { fetch: true });
    notMerged = !(st && (st.merged === true || st.state === 'merged'));
    if (!notMerged) reasons.push(branch + ' 는 이미 ' + base + ' 에 병합됐다 — 끝난 작업이지 재개 대상이 아니다');
  } catch {
    // Unknown is not a pass. Resuming a branch whose state we cannot read is
    // exactly the case this function exists to refuse.
    notMerged = false;
    reasons.push(branch + ' 의 병합 상태를 판정할 수 없다 — 모르는 상태에서 재개하지 않는다');
  }

  return { owned: record && forkedFromBase && notMerged, checks: { record, forkedFromBase, notMerged }, reasons };
}

// ── Module Exports ───────────────────────────────────────

module.exports = { resolveBase, canCommitTo, mergedState, branchName, preflight, entryDecision, verifyOwnership };
