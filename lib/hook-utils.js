...
function readJsonInput() {
  ...
  // A `--flag` is a hook option (post-tool-task.js --failed), not a payload —
  // treating it as one skipped stdin and lost the real input.
...
/**
 * Make `<root>/.lens` and keep it out of git (v3.48).
 *
 * Runtime state landed in commits and kept repos "modified" — 실측 snapholo 41
 * tracked `.lens` files, 9 repos affected. The exclusion goes to the repo's own
 * `info/exclude` (local only, nothing to commit). `--git-path` resolves it for a
 * worktree whose `.git` is a file. Not a repo, or git missing → just the folder.
 * Returns the directory, or null when it could not be made.
 */
function ensureLensDir(root) {
  if (!root) return null;
  const dir = path.join(root, '.lens');
  if (!safeEnsureDir(dir)) return null;
  try {
    const r = require('child_process').spawnSync('git', ['-C', root, 'rev-parse', '--git-path', 'info/exclude'], {
      encoding: 'utf-8', timeout: 3000, windowsHide: true, stdio: ['ignore', 'pipe', 'ignore'],
    });
    const rel = r.status === 0 ? String(r.stdout || '').trim() : '';
    if (rel) {
      const exclude = path.resolve(root, rel);
      const text = fs.existsSync(exclude) ? fs.readFileSync(exclude, 'utf-8') : '';
      if (!text.split(/\r?\n/).some(l => l.trim() === '.lens/')) {
        safeEnsureDir(path.dirname(exclude));
        fs.appendFileSync(exclude, `${text && !text.endsWith('\n') ? '\n' : ''}.lens/\n`, 'utf-8');
      }
    }
  } catch { /* exclusion is best-effort; the folder is what the caller needs */ }
  return dir;
}
