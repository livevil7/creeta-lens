...
test('Codex JSON parsers normalize installed plugins and marketplaces', () => {
  const plugins = parseCodexPluginList(JSON.stringify({
    installed: [{
      pluginId: 'lens@CreetaCorp',
      marketplaceName: 'CreetaCorp',
      version: '3.26.0',
      installed: true,
      enabled: true,
      source: { source: 'local', path: 'C:/codex/lens' },
      marketplaceSource: { sourceType: 'git', source: 'https://github.com/livevil7/creeta-lens.git' },
    }],
    available: [],
  }));
  assert.strictEqual(plugins.length, 1);
  assert.strictEqual(plugins[0].id, 'lens@CreetaCorp');
  assert.strictEqual(plugins[0].marketplace, 'CreetaCorp');
  assert.strictEqual(plugins[0].enabled, true);

  const marketplaces = parseCodexMarketplaceList(JSON.stringify({
    marketplaces: [{ name: 'personal' }, { name: 'CreetaCorp' }],
  }));
  assert.deepStrictEqual(marketplaces, ['personal', 'CreetaCorp']);
});

test('runtime-specific manifest paths do not cross host state', () => {
  assert.ok(getManifestPath('claude').replace(/\\/g, '/').endsWith('/.claude/lens/manifest.json'));
  assert.ok(getManifestPath('codex').replace(/\\/g, '/').endsWith('/.codex/lens/manifest.json'));
});
