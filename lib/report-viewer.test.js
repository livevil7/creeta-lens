...
test('showReport: an existing deck is opened directly, with no preview written', () => {
  const root = makeRepo({
    [`docs/tasks/${PLAN}.md`]: '# plan',
    [`docs/tasks/${PLAN}.html`]: '<h1>plan</h1>',
  });
  const res = showReport(root, PLAN, { platform: 'win32', env: LOCAL, runner: runnerReturning({ status: 0 }) });
  assert.strictEqual(res.viewer, `docs/tasks/${PLAN}.html`);
  assert.strictEqual(fs.existsSync(path.join(root, '.lens', 'preview')), false);
});
