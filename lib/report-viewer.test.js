 ...
 *   4. the markdown is the document (v3.39) — a legacy HTML deck is only a fallback;
...
/**
 * Fake spawnSync. Association probes (`assoc` / `reg query`) answer per
 * `associated`; every other call is the actual open and is recorded in `calls`.
 */
function runnerReturning(result, { associated = true } = {}) {
  const calls = [];
  const fn = (cmd, args, opts) => {
    const argv = (args || []).join(' ');
    if (/\bassoc\b/.test(argv)) {
      return associated ? { status: 0, stdout: '.html=htmlfile' } : { status: 1, stdout: '' };
    }
    if (cmd === 'reg.exe') return associated ? { status: 0, stdout: 'ProgId' } : { status: 1, stdout: '' };
...
// --- 4b. windows association + markdown preview (v3.37.1) --------------------
test('openFile: an unassociated extension on Windows fails instead of claiming success', () => {
  const root = makeRepo({ [`docs/tasks/${PLAN}.md`]: '# plan' });
  const runner = runnerReturning({ status: 0 }, { associated: false });
  const res = showReport(root, PLAN, { platform: 'win32', env: LOCAL, runner });
  assert.strictEqual(res.ok, false, 'start returns 0 while opening nothing — that is not success');
  assert.strictEqual(res.method, 'failed');
  assert.match(res.note, /연결 프로그램/);
  assert.strictEqual(runner.calls.length, 0, 'nothing should be handed to the opener');
  assert.strictEqual(wasShown(root, PLAN), null);
});

test('showReport: a deck-less plan is wrapped in a preview page, and the page is what opens', () => {
  const root = makeRepo({ [`docs/tasks/${PLAN}.md`]: '# plan\n\n<script>alert(1)</script>\n' });
  const runner = runnerReturning({ status: 0 });
  const res = showReport(root, PLAN, { platform: 'win32', env: LOCAL, runner });
  assert.strictEqual(res.ok, true);
  assert.strictEqual(res.file, `docs/tasks/${PLAN}.md`, 'the md stays the recorded source of truth');
  assert.strictEqual(res.viewer, `.lens/preview/${PLAN}.html`);
  assert.ok(runner.calls[0].args.join(' ').endsWith(path.join('.lens', 'preview', `${PLAN}.html`)));
  const page = fs.readFileSync(path.join(root, '.lens', 'preview', `${PLAN}.html`), 'utf-8');
  assert.ok(page.includes('&lt;script&gt;'), 'document text must be escaped, not executed');
  assert.ok(wasShown(root, PLAN));
});

test('showReport: the markdown is rendered even when a legacy deck sits beside it', () => {
  const root = makeRepo({
    [`docs/tasks/${PLAN}.md`]: '# plan\n\n| a | b |\n|---|---|\n| 1 | 2 |\n',
    [`docs/tasks/${PLAN}.html`]: '<h1>old deck</h1>',
  });
  const res = showReport(root, PLAN, { platform: 'win32', env: LOCAL, runner: runnerReturning({ status: 0 }) });
  assert.strictEqual(res.viewer, `.lens/preview/${PLAN}.html`);
  const page = fs.readFileSync(path.join(root, '.lens', 'preview', `${PLAN}.html`), 'utf-8');
  assert.ok(page.includes('<table>'), 'the preview is rendered, not raw text');
  assert.ok(!page.includes('<pre>| a'), 'no raw markdown wall');
});

// --- 4c. platform-native surfaces + staleness (v3.39) ------------------------
...
test('recordShown: an unknown method or a missing ref is refused', () => {
  ...
  assert.strictEqual(recordShown(root, PLAN, 'browser', 'x').ok, false, 'browser is performed by showReport, not claimed');
  assert.strictEqual(wasShown(root, PLAN), null);
});
