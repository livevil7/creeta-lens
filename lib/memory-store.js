/**
 * Lens - Memory Store Module
 * Persists session data across Claude Code and Codex sessions.
 */

const fs = require('fs');
const path = require('path');
const os = require('os');
const { safeEnsureDir, safeReadJson, safeWriteJson } = require('./hook-utils');

const DEFAULT_MEMORY_FILE = '.lens-memory.json';

/**
 * Get memory file path.
 * Priority: config override → plugin data dir → host home/lens/
 */
function getMemoryPath(configPath) {
  if (configPath) {
    const dir = path.dirname(configPath);
    if (fs.existsSync(dir)) return configPath;
  }

  // Plugin hooks receive a host-owned writable data directory. Prefer it so
  // Codex never writes state into the immutable plugin cache.
  const pluginData = process.env.PLUGIN_DATA || process.env.CLAUDE_PLUGIN_DATA;
  if (pluginData) return path.join(pluginData, DEFAULT_MEMORY_FILE);

  // Codex skill/runtime state.
  if (process.env.CODEX_HOME || process.env.PLUGIN_ROOT) {
    const codexHome = process.env.CODEX_HOME || path.join(os.homedir(), '.codex');
    return path.join(codexHome, 'lens', DEFAULT_MEMORY_FILE);
  }

  // Stable Claude Code path.
  const claudeHome = process.env.CLAUDE_HOME || path.join(os.homedir(), '.claude');
  return path.join(claudeHome, 'lens', DEFAULT_MEMORY_FILE);
}

/**
 * Load memory from file.
 */
function loadMemory(configPath) {
  const memPath = getMemoryPath(configPath);
  return safeReadJson(memPath, createDefaultMemory()) || createDefaultMemory();
}

/**
 * Save memory to file.
 */
function saveMemory(memory, configPath) {
  const memPath = getMemoryPath(configPath);
  safeEnsureDir(path.dirname(memPath));
  return safeWriteJson(memPath, memory, { atomic: true });
}

/**
 * Create default memory structure.
 */
function createDefaultMemory() {
  return {
    version: '1.0.0',
    sessionCount: 0,
    lastSessionAt: null,
    lastUsedSkills: [],
    recommendationHistory: [],
    skillUsageCount: {},
  };
}

/**
 * Record a new session start.
 */
function recordSessionStart(memory) {
  memory.sessionCount = (memory.sessionCount || 0) + 1;
  memory.lastSessionAt = new Date().toISOString();
  return memory;
}

/**
 * Record a skill usage.
 */
function recordSkillUsage(memory, skillName) {
  // Update last used
  memory.lastUsedSkills = memory.lastUsedSkills || [];
  memory.lastUsedSkills = [skillName, ...memory.lastUsedSkills.filter(s => s !== skillName)].slice(0, 10);

  // Update count
  memory.skillUsageCount = memory.skillUsageCount || {};
  memory.skillUsageCount[skillName] = (memory.skillUsageCount[skillName] || 0) + 1;

  return memory;
}

/**
 * Record a recommendation.
 */
function recordRecommendation(memory, request, recommendedSkills) {
  memory.recommendationHistory = memory.recommendationHistory || [];
  memory.recommendationHistory.unshift({
    request: request.substring(0, 100),
    skills: recommendedSkills,
    at: new Date().toISOString()
  });
  // Keep last 20 recommendations
  memory.recommendationHistory = memory.recommendationHistory.slice(0, 20);
  return memory;
}

/**
 * Record a plan creation.
 */
function recordPlanCreation(memory, planId, taskSummary) {
  memory.planHistory = memory.planHistory || [];
  memory.planHistory.unshift({
    planId,
    task: (taskSummary || '').substring(0, 100),
    at: new Date().toISOString(),
  });
  memory.planHistory = memory.planHistory.slice(0, 20);
  return memory;
}

/**
 * Get top used skills.
 */
function getTopSkills(memory, limit = 5) {
  const counts = memory.skillUsageCount || {};
  return Object.entries(counts)
    .sort((a, b) => b[1] - a[1])
    .slice(0, limit)
    .map(([name, count]) => ({ name, count }));
}

/**
 * Format memory summary for session context.
 */
function formatMemorySummary(memory) {
  const lines = [];

  lines.push(`Session #${memory.sessionCount || 1}`);

  if (memory.lastSessionAt) {
    const last = new Date(memory.lastSessionAt);
    lines.push(`Last session: ${last.toLocaleDateString()}`);
  }

  if (memory.lastUsedSkills && memory.lastUsedSkills.length > 0) {
    lines.push(`Recently used: ${memory.lastUsedSkills.slice(0, 5).map(s => '/' + s).join(', ')}`);
  }

  const topSkills = getTopSkills(memory, 3);
  if (topSkills.length > 0) {
    lines.push(`Most used: ${topSkills.map(s => `/${s.name}(${s.count})`).join(', ')}`);
  }

  return lines.join('\n');
}

module.exports = {
  loadMemory,
  saveMemory,
  recordSessionStart,
  recordSkillUsage,
  recordRecommendation,
  recordPlanCreation,
  getTopSkills,
  formatMemorySummary,
};
