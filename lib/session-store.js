...
function update(kind, mutate, fallback = null, sessionId) {
  ...
  const run = () => {
    ...
    // A write that did not land returns null: callers acting on the new state
    // (the Stop block counter) must not act on a count that was never saved.
    return safeWriteJson(file, next) ? next : null;
  };
  try {
