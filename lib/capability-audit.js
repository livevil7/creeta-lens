...
function getProjectRoot(projectDir) {
  return projectDir || process.env.CODEX_PROJECT_DIR || process.env.CLAUDE_PROJECT_DIR || process.cwd();
}
...
function isLensRepo(root) {
  try {
    ...
    return PLUGIN_MANIFEST_RELS.some((rel) => {
      const manifest = safeReadJson(path.join(root, rel), null);
