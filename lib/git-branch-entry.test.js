...
function mergeAndDelete(repo, branch, { noFf = true, message = null } = {}) {
  run(repo, 'push', '-u', 'origin', branch);
  run(repo, 'checkout', 'master');
  if (message) run(repo, 'merge', '--no-ff', '-m', message, branch);
  else if (noFf) run(repo, 'merge', '--no-ff', '--no-edit', branch);
...
test('a merge of fix/foo is not evidence for a branch named foo', () => {
  const repo = newRepo({ branch: 'fix/foo' });
  mergeAndDelete(repo, 'fix/foo');
  const st = mergedState(repo, 'foo', 'master');
  assert.strictEqual(st.state, 'unknown', st.reason);
});

test('a GitHub PR merge message (from <owner>/<branch>) is still evidence', () => {
  const repo = newRepo({ branch: 'fix/pr' });
  mergeAndDelete(repo, 'fix/pr', { message: 'Merge pull request #7 from livevil7/fix/pr' });
  const st = mergedState(repo, 'fix/pr', 'master');
  assert.strictEqual(st.state, 'merged-deleted', st.reason);
  // ...but it does not prove the last path segment alone
  assert.strictEqual(mergedState(repo, 'pr', 'master').state, 'unknown');
});
