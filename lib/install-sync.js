 ...
 *
 * A per-user manifest at ~/.claude/lens/manifest.json declares which plugins
 * SHOULD be installed (managed) and which SHOULD be removed (excluded). This
 * module diffs that manifest against the machine's actual install state and
 ...
 * runs `claude plugin install` / `uninstall` — that is the SKILL.md's job.
 *
 * Usage:
 *   node lib/install-sync.js --manifest-path            print manifest path (create empty if absent)
...
const LENS_DIR = path.join(os.homedir(), '.claude', 'lens');
...
function runClaude(args) {
  ...
  const isWin = process.platform === 'win32';
  const useShell = isWin && /\.(cmd|bat)$/i.test(claude);
  try {
    let p;
    if (useShell) {
      const quoted = [`"${claude}"`, ...args.map((a) => `"${a}"`)].join(' ');
      p = spawnSync(quoted, {
        encoding: 'utf-8', timeout: 120000, windowsHide: true, shell: true,
      });
    } else {
      p = spawnSync(claude, args, {
...
function main(argv) {
  ...
  const manifestPath = valAfter('--manifest-path-file') || MANIFEST_PATH;
  ...
  console.error(
    'usage: install-sync.js --manifest-path | --list-installed [--json] | ' +
