...
test('인벤토리가 없으면 파생 실패 — 조용히 짧은 목록을 내지 않는다', () => {
  ...
  assert.strictEqual(r.valid, false);
  assert.match(r.hints.join(' '), /Phase 2\.45/);
  assert.ok(!/Phase/.test(r.problems.join(' ')), 'the person-facing layer carries no phase jargon');
});

test('성공 기준이 없으면 파생 실패', () => {
  const r = deriveTodoItems(PLAN.replace('## 🎯 What — 목표', '## 서문'));
  assert.strictEqual(r.valid, false);
  assert.match(r.hints.join(' '), /Phase 0/);
  assert.ok(!/Phase/.test(r.problems.join(' ')));
});

// v3.39: steps are optional. A plan is still executable from goals + inventory.
test('Plan A 제목이 없으면 🛠 How 블록에서 단계를 읽는다 (Plan B 는 자른다)', () => {
...
test('방법 섹션이 아예 없어도 파생은 된다 — 단계 없음은 경고', () => {
  const r = deriveTodoItems(PLAN.replace('## 🛠 How', '## 부록').replace('### Plan A — 권장 경로', '### 나중에'));
  assert.strictEqual(r.valid, true);
  assert.strictEqual(r.steps.length, 0);
  assert.strictEqual(r.total, 4);
  assert.match(r.warnings.join(' '), /찾지 못했다/);
});
...
test('단계가 비고 트리거 불릿만 있으면 그것을 단계로 승격하지 않는다', () => {
  ...
  // v3.39: still no promotion — but an empty step list is a warning now, not a refusal.
  assert.strictEqual(r.valid, true);
  assert.match(r.warnings.join(' '), /체크박스 단계가 없다/);
});
...
// ── v3.39 — false positives, deferred rows, plan kinds ──────
//
// Owner, 2026-09-14: "update todo 하면 뭐가 계속 없다고만 찡찡거리던데? 이게 맞아?"
// Measured across the workspace's 105 plans: 57 failed the structure gate and 86
// failed the todo gate, much of it on well-formed documents. These pin the four
// false positives and the two new rules (plan kind, deferred status).

console.log('\n== v3.39 — false positives and plan kinds ==');

test('a pipe inside a code span stays in its cell', () => {
  const r = validatePlanCoverage(ledger('| 1 | `grep -rn "a|b"` 로 확인 | 조사 | 검증 2 | 포함 |'));
  assert.deepStrictEqual(r.problems, []);
  assert.strictEqual(r.included, 1);
});

test('a raw pipe in the item text does not push the status out of its column', () => {
  const r = validatePlanCoverage(ledger('| 1 | 스킬 A | B 정리 | 조사 | How C-1 | 포함 |'));
  assert.deepStrictEqual(r.problems, []);
  assert.strictEqual(r.included, 1);
});

test('"포함 안 함" is not an inclusion', () => {
  const r = validatePlanCoverage(ledger('| 1 | a | 조사 | Plan A | 포함 안 함 |'));
  assert.strictEqual(r.included, 0);
  assert.match(r.problems.join(' '), /해석할 수 없다/);
});

test('"보류: … (pending)" is deferred — not included through the "in" of pending', () => {
  const r = validatePlanCoverage(ledger('| 1 | a | 조사 | — | 보류: 다음 분기 (pending) |'));
  assert.strictEqual(r.included, 0);
  assert.strictEqual(r.deferred, 1);
  assert.deepStrictEqual(r.problems, []);
});

test('보류 without a reason is refused like 제외', () => {
  const r = validatePlanCoverage(ledger('| 1 | a | 조사 | — | 보류 |'));
  assert.match(r.problems.join(' '), /보류 사유가 없다/);
});

test('포함 with a leading symbol and a trailing note is still an inclusion', () => {
  const r = validatePlanCoverage(ledger('| 1 | a | 조사 | How | ✅ 포함 (우선순위 최하) |'));
  assert.strictEqual(r.included, 1);
  assert.deepStrictEqual(r.problems, []);
});

test('deferred rows do not become todo items', () => {
  const r = deriveTodoItems(PLAN.replace(
    '| 2 | 인증메일 발송 | Codex 조사 | Plan A step2 | 포함 |',
    '| 2 | 인증메일 발송 | Codex 조사 | — | 보류: 메일 업체 계약 후 |'));
  assert.deepStrictEqual(r.inventory, ['가입 폼 검증']);
});

const FLOOR = extra => [
  '## 🎯 목표', '- 가입을 끝까지 마칠 수 있다', '',
  '## ❓ 왜', '지금은 중간에 끊긴다', '',
  ledger('| 1 | 폼 검증 | 요청 | 어떻게 | 포함 |'),
  '## 🛠 어떻게', '- [ ] 폼 검증 붙이기', '',
  '## ✅ 검증', '| # | 신호 |', '|---|---|', '| 1 | 가입 완료 화면 |', '',
  '## 🚫 건드리지 않는 것', '- 결제 모듈', '',
  extra,
...
test('`{id}` and {job_uuid} inside sentences are not unfilled templates', () => {
  const r = validatePlanStructure(FLOOR('계획서는 `{id}` 와 {job_uuid} 로 부른다.'), 'standard');
  assert.strictEqual(r.valid, true, r.missing.join(','));
  assert.match(r.warnings.join(' '), /중괄호/);
});

test('a bullet that is nothing but a placeholder is still refused', () => {
  const r = validatePlanStructure(FLOOR('- {사람 언어 목표 1 — 예: "가입"}'), 'standard');
  assert.ok(r.missing.some(m => /placeholder/.test(m)), r.missing.join(','));
});

test('a table cell that is nothing but a placeholder is refused', () => {
  const r = validatePlanStructure(FLOOR('| ID | 리스크 |\n|---|---|\n| R1 | {무엇이 잘못될 수 있나} |'), 'standard');
  assert.ok(r.missing.some(m => /placeholder/.test(m)), r.missing.join(','));
});

test('placeholders inside a code fence are ignored', () => {
  const r = validatePlanStructure(FLOOR('```text\n{id}\n- {x}\n```'), 'standard');
  assert.strictEqual(r.valid, true, r.missing.join(','));
});

test('개선 plans must carry AS-IS → TO-BE', () => {
  const doc = `---\nkind: 개선\ncreated: 2026-09-14\n---\n\n${FLOOR('')}`;
  const r = validatePlanStructure(doc, 'standard');
  assert.strictEqual(r.kind, '개선');
  assert.ok(r.missing.includes('AsIsToBe'), r.missing.join(','));
  const ok = validatePlanStructure(`${doc}\n## AS-IS → TO-BE\n| 지금 | 바뀐 뒤 |\n|---|---|\n| 3단계 | 1단계 |\n`, 'standard');
  assert.strictEqual(ok.valid, true, ok.missing.join(','));
});

test('kind aliases read as 개선, an unknown kind falls back to 신규', () => {
  assert.strictEqual(normalizeKind('improve'), '개선');
  assert.strictEqual(normalizeKind('변경'), '개선');
  assert.strictEqual(normalizeKind('xyz'), null);
  const r = validatePlanStructure(`---\nkind: xyz\n---\n\n${FLOOR('')}`, 'standard');
  assert.strictEqual(r.kind, '신규');
  assert.ok(!r.missing.includes('AsIsToBe'));
});

test('an explicit kind argument wins over frontmatter', () => {
  const r = validatePlanStructure(`---\nkind: 신규\n---\n\n${FLOOR('')}`, 'standard', 'improve');
  assert.ok(r.missing.includes('AsIsToBe'));
});

test('조사보고 answers a question — no inventory, method or DO NOT CHANGE required', () => {
  const doc = '---\nkind: 조사보고\n---\n\n## 🎯 질문\n- 어느 쪽이 싼가\n\n## 📊 근거\n- a\n\n## 💡 결론\n- b\n';
  const r = validatePlanStructure(doc, 'deep');
  assert.strictEqual(r.valid, true, r.missing.join(','));
  assert.ok(!validatePlanStructure('---\nkind: 조사보고\n---\n\n## 🎯 질문\n- a\n', 'deep').valid);
});

test('nested bullets under a goal are details, not more goals', () => {
  const doc = PLAN.replace('- 방문자가 이메일로 가입을 끝까지 마칠 수 있다',
    '- 방문자가 이메일로 가입을 끝까지 마칠 수 있다\n  - 폼 3칸\n  - 약관 동의');
  assert.strictEqual(deriveTodoItems(doc).goals.length, 2);
});

test('🚧 비목표 written before 🎯 목표 is not read as the goal', () => {
  const doc = PLAN.replace('## 🎯 What — 목표', '## 🚧 비목표\n- 소셜 로그인 붙이기\n\n## 🎯 What — 목표');
  const r = deriveTodoItems(doc);
  assert.ok(!r.goals.some(g => g.includes('소셜')), r.goals.join(' | '));
  assert.strictEqual(r.goals.length, 2);
});

const INV = ['## 📋 작업 인벤토리', '', '| # | 작업 항목 | 출처 | 반영 위치 | 상태 |', '|---|---|---|---|---|', '| 1 | 폼 검증 추가 | 요청 | 어떻게 | 포함 |', ''];

test('a plan without "Plan A" reads checkboxes under 🛠 어떻게 and stops at Plan B', () => {
  const doc = ['## 🎯 목표', '- 하나', '', ...INV,
    '## 🛠 어떻게', '', '### A. 묶음', '- [ ] 첫 일', '- 설명 불릿', '', '### Plan B', '- [ ] 우회', ''].join('\n');
  const r = deriveTodoItems(doc);
  assert.deepStrictEqual(r.steps, ['첫 일']);
  assert.strictEqual(r.valid, true);
});

test('prose-only method: no steps is a warning, not a refusal', () => {
  const doc = ['## 🎯 목표', '- 하나', '', ...INV, '## 🛠 어떻게', '', '- **A-1** 골격을 계약으로', ''].join('\n');
  const r = deriveTodoItems(doc);
  assert.deepStrictEqual(r.steps, []);
  assert.strictEqual(r.valid, true);
  assert.match(r.warnings.join(' '), /체크박스 단계가 없다/);
...
test('a step that repeats an inventory item is not registered twice', () => {
  const doc = ['## 🎯 목표', '- 하나', '', ...INV, '## 🛠 어떻게', '- [ ] step 1: 폼 검증 추가', '- [ ] 배포', ''].join('\n');
  assert.deepStrictEqual(deriveTodoItems(doc).steps, ['배포']);
});

test('the v3.39 plan itself (template-free shape) derives and passes', () => {
  const fs = require('fs');
  const path = require('path');
  const file = path.join(__dirname, '..', 'docs', 'tasks', '2026-09-14-cp-native-artifacts.md');
  if (!fs.existsSync(file)) return; // archived by /cd — nothing to pin
  const c = fs.readFileSync(file, 'utf-8');
  const r = deriveTodoItems(c);
  assert.strictEqual(r.valid, true, r.problems.join(' / '));
  assert.ok(r.goals.length >= 6, `goals=${r.goals.length}`);
  assert.ok(r.inventory.length >= 70, `inventory=${r.inventory.length}`);
  assert.strictEqual(validatePlanStructure(c, 'standard').valid, true);
  assert.strictEqual(validatePlanCoverage(c).valid, true);
});
