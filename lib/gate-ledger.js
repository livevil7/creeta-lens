 ...
 *
 * unlazy ships 6. Lens used 3 because a blocked turn delivered NO message to the
 * user, and the user's standing 2-minute progress rule (harness-rules §4.4)
 * makes silence itself a violation. v3.39: 2 — and hooks/stop.js now attaches a
 * user-visible systemMessage to every block, so the turns are no longer silent.
 * The counter is keyed by content hash, so any real edit to the ledger resets it
 * — the cap only bites when nothing is moving.
 */
const MAX_BLOCKS = 2;
