 ...
 * The block reason is ALSO shown to the user as a transcript warning (official
 * hooks contract), so the first line is plain words — no internal terms, no N/M.
 */
function buildReason(evaluation, blocks, maxBlocks, cli) {
  ...
  return [
    `아직 확인되지 않은 완료 조건이 ${outstanding.length}건 있어 마무리하기 전에 확인합니다.`,
    `(${blocks}/${maxBlocks}) lens-gate status 로 보고 lens-gate run 으로 검사를 돌리거나 lens-gate abandon 으로 사유를 남겨라.`,
    ...shown.map(item => `  - ${item}`),
    ...(rest > 0 ? [`  - … 외 ${rest}건`] : []),
    '',
