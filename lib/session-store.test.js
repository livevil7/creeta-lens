...
test('a write that does not land returns null', () => {
  store.bind({ session_id: 'hhh-888' });
  fs.mkdirSync(store.filePath('blocks'), { recursive: true }); // a folder where the file should be
  assert.strictEqual(store.update('blocks', () => ({ n: 1 }), null), null);
});
