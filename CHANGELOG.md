...
- **페이지에서 「지금 실행」을 골라도 실행으로 이어지지 않을 수 있던 것.** 대표 지적: *"지금 아티팩트에서 내가 승인하면 자동으로 /cc로 실행이 되어야 하는거 아니야?"* 페이지 답은 `artifact-auto-react` 알림으로 오는데, 그 알림은 "NOT USER INPUT · 승인으로 해석하지 마라" 포장을 두르고 오고 답 내용도 싣지 않는다(3.50.1 배포 뒤 실측). 스킬이 이것을 가리지 않아 모델이 페이지 승인을 받고도 채팅으로 다시 물을 수 있었다. `skills/cp/SKILL.md` Phase 6 「페이지 답변」 — `ArtifactComments read` 로 스레드를 읽고, 머리표 `[the user (owner), posted by the artifact, sent to you]` 이면 대표 본인의 답으로 받아 **`지금 실행` 이면 그 턴에 바로 `/cc`** 로 넘긴다. `the user` 가 아닌 편집자의 답은 승인으로 치지 않는다. 자동 답글이 이미 있으면 답글을 또 쓰지 않는다.
- 승인 문항 「지금 실행」 설명이 "보내면 바로 실행을 시작합니다" 로 시작한다(`templates/cp-questions.html` 예시 · Phase 4.5).
- `docs/rules/harness-rules.md` §4.13 「페이지 승인 = 실행」.
...
- `templates/cp-questions.html` — 계획서 페이지에 그대로 붙이는 질문 블록. JSON 하나로 객관식(`choice`)·복수 선택(`multi`)·주관식(`text`)·「기타 — 직접 적기」·선택지별 적는 칸을 그린다. [답변 보내기] 한 번이 `comments` capability 의 `sendToClaude` 로 답을 댓글 한 건(`[Lens /cp 답변] <plan_id>` + 줄마다 `[id] 질문 → 답`)으로 세션에 보낸다. 보낼 수 없는 화면(권한·세션 없음)이면 같은 글을 복사 칸에 보여 채팅으로 받는다. 360px 에서 가로 넘침 없음.
- `templates/cp-questions.test.js` — 답 글의 모양(표지 줄 · 질문마다 한 줄 · `(답 없음)` · 제어문자 제거)과 예시 JSON 의 승인 선택지 셋을 고정. 6/6.

### Changed (v3.50.0)

- `skills/cp/SKILL.md` 계약 카드 8 — artifact 레인이면 승인 · 🙋 대표 결정 · 모호성 질문 · deep D2 질문을 페이지 질문 블록으로 묻고 질문창은 띄우지 않는다(같은 질문이 두 곳이면 답의 정본이 갈린다). 발행 결과에 watch 가 없을 때만 질문창을 같이 띄운다. Codex·sendfile 레인은 종전대로.
- Phase 4.5 — 「질문은 페이지 안에서」 절 신설(블록은 붙이기만 · 싣는 것 · 발행마다 `capabilities: {comments: {}}` · 답한 질문은 다음 판에서 빼고 `🧭 결정` 으로).
- Phase 5.1 — 보고에 `🗳 답하는 곳` 줄. Phase 6 — 「페이지 답변」 처리(`approved_via: artifact`, `(답 없음)` 은 추천안으로 가정하지 않는다, 스레드에 반영 내용 reply → resolve).
- `docs/rules/harness-rules.md` §4.13 신설.

## [3.49.0] - 2026-09-23

**계획서를 다시 읽히는 페이지로 띄운다.** 대표 지적(2026-09-23): *"기획안 정리하는 모듈을 뭐 어떻게 건드렸길래 저렇게 갑자기 그냥 md파일 쓰레기처럼 만드는걸로 변경이 되었어? 개 쓰레긴데? 하나도 알아볼수가 없는데?"* · *"html을 만들지 말라고 한게 개 쓰레기 규칙인데?"*

2026-09-16 대표 지시 *"보드나 html 이건 안 해도 돼, 쓸데없는 짓"* 은 **슬라이드 덱·flow 보드**를 가리킨 것이었는데, 두 릴리스에 걸쳐 계획서 표시 자체로 번졌다:

- v3.42(`d413fc7`) — 표시 레인에서 HTML 을 통째로 금지하고 `lib/md-render.js`(md→페이지 렌더러 278줄)를 삭제, report-viewer 의 브라우저 레인·OS opener 제거.
- v3.48(`939a674`) — artifact 레인을 `artifact-design` 로드 → 읽히는 페이지 발행에서 **"md 파일을 그대로 Artifact 로 발행한다"** 로 바꾸고, 페이지 구성 규칙(결정 블록 · 읽는 순서 · frontmatter 배지)을 삭제. 그 결과 대표 화면에 frontmatter 덤프가 붙은 md 원문이 떴다.

### Changed (v3.49.0)

- `skills/cp/SKILL.md` Phase 4.5 artifact 레인 — **`artifact-design` 스킬 로드 후 읽히는 HTML 페이지로 발행**. `.md` 원문을 그대로 Artifact 로 발행하지 않는다.
- `skills/cp/SKILL.md` — **페이지에 담는 것** 블록 복원(① 맨 위 결정 블록 ② 목표→왜→AS-IS/TO-BE→인벤토리 전량→어떻게→검증→리스크 ③ frontmatter 는 배지 한 줄, 원문 덤프 금지).
- 계약 카드 6 — "HTML 을 만들지 않는다" → **"md 는 저장, 화면은 읽히는 페이지"**. 금지 범위를 원래 지시대로 **덱·보드·`flow.html`** 로 좁혔다.
- 재발행 대상 경로를 `<id>.html` 로 명시 — 같은 링크 유지 규칙은 그대로.
- 스킬 frontmatter `description` 의 *"Lens itself renders no HTML"* → *"Lens builds no slide decks or boards"*.

### 그대로 두는 것 (v3.49.0)

- 슬라이드 덱·보드(v3.39 폐지)와 `/cps` 의 `docs/rules/flow.html`(v3.42) — 이쪽이 원래 지시가 가리킨 대상이다.
- `lib/report-viewer.js` 는 기록 전용 그대로(sendfile 레인은 원격·헤드리스용으로 md 를 그대로 보낸다).
...
- Windows 에서 `lens-gate run` 의 검사가 백그라운드 자식을 남기고 먼저 끝나면, 시간 초과 때 결과는 제때 내지만 그 고아 프로세스는 스스로 끝날 때까지 남는다(부모가 사라지면 `taskkill /T` 가 닿지 않는다). 검사 명령은 백그라운드 프로세스를 띄우지 않는다 — `/cc` 0.5 규칙.

## [3.47.0] - 2026-09-20

**`/cs` 가 레포만 맞추고 있었다 — 이제 머신 자체를 맞춘다.** 대표 질문: *"내가 /cs 를 하면 지금 이런 리포 뿐만 아니고 모든 게 다 동기화되게 하는 거지?"* 그래야 맞다.

레포만 같고 플러그인·스킬·도구가 머신마다 다르면 **같은 명령이 머신마다 다르게 동작한다.** 2026-09-20 5대 실측이 그 증거다: Lens 가 `3.37.1 / 3.38.0 / 3.40.0 / 없음 / 3.45.0` 으로 다섯 대가 전부 달랐고, macOS 두 대에서는 `/cs` 자신이 레포를 **0개**로 세면서 성공을 보고했다.

### Added (v3.47.0)

- **T9 함대 동기화** — `/cs` 가 레포 동기화 끝에 `livevil-setting/scripts/fleet-sync.sh` 를 같은 런에서 돌린다. 플러그인(설치·갱신·제외목록 제거) · 개인 스킬 · 필수 CLI 점검이 한 번에 끝난다. 결과는 `🧩 함대 동기화` 버킷으로 요약만 싣는다 — 전체를 쏟으면 레포 리포트가 묻힌다.
- 끄기: `LENS_SYNC_FLEET=0`. `pull` 액션에서는 돌지 않는다.

### Changed (v3.47.0)

- **SoT 는 Lens 가 아니라 사용자의 `livevil-setting/claude-code/skills.json`** 이다. 플러그인 목록은 사용자 자산이지 이 플러그인의 것이 아니다 — Lens 는 그 선언을 실행할 뿐이고, 파일이 없으면 조용히 건너뛴다.
- 구조는 **pull 형**이다. 중앙 머신이 밀지 않는다: 꺼진 머신은 건너뛰어져 조용히 뒤처지고(실측: mac-001 이 242커밋 뒤처졌는데 fetch 전에는 `0/0` 으로 "최신"이었다), macmini 는 crontab 이 TCC 로 원격 편집이 막히며, Windows 는 SSH 세션에서 reparse point 통과가 막힌다(`os error 448`). 각자 당기면 이 셋이 전부 사라지고 허브에 자격증명이 모이지도 않는다.

## [3.46.2] - 2026-09-20

**`/cu` 가 매 실행마다 오류처럼 보이는 경고를 냈고, 동시에 검사하지 않은 소스를 조용히 감췄다.** 대표 지적: *"매번 컴퓨터마다 돌릴 때마다 뭐가 오류로 맨날 안 한대"* → *"스킵하는 게 오류지. 왜 스킵하는 건데?"*

### Fixed (v3.46.2)

- **winget 요약줄 스킵 경고 (한글 로케일)** — 요약줄 판정이 `\d+ upgrades? available\.?` 영어 정규식 하나뿐이라, 한글 Windows 의 `25 업그레이드를 사용할 수 있습니다.` 가 그 가드를 빠져나가 패키지 행 파서까지 흘러갔다. Id 셀이 비어 매 실행 `cu: winget 행 파싱 건너뜀: ...` 를 stderr 로 뱉었다. **실제 누락은 0** 이었지만(25건 표시에 25건 파싱) 사용자에게는 고장으로 읽혔다. 이제 **표의 Id 컬럼에 닿지 않는 줄은 로케일과 무관하게 조용히 거른다** — 폭으로 판정하므로 어떤 언어의 요약문에도 걸린다. 실측: sj-omen·sj-x1 둘 다 경고 소멸, 항목 수 불변(35건).
- **도구가 PATH 에 없으면 소스가 통째로 사라지던 것** — 각 스캐너는 실행 파일을 못 찾으면 `([], None)` 을 돌려준다. 오류도 아니고 항목도 없으니 리포트에서 그 소스가 **아예 없던 것처럼** 빠지고, 사용자는 "검사했는데 올릴 게 없다" 로 읽는다. 실측(2026-09-20): 두 macOS 머신의 비로그인 셸에서 brew·npm·pip·code 가 전부 PATH 밖이라 **13건(플러그인만)** 을 내면서 `source_errors` 는 0 이었다 — brew 패키지 수십 개가 검사조차 되지 않았다. 이제 `sources_skipped[]` 로 **검사하지 않았다는 사실을 밝힌다**. 플랫폼상 없는 게 당연한 조합(macOS 의 winget, Windows 의 brew)은 말하지 않는다.

### Added (v3.46.2)

- `scan` 출력에 **`sources_skipped[]`** 필드 (additive — 기존 키·순서 불변). `{source, reason}` 형태.
- `_source_tool()` · `_source_applies_here()` — 소스별 실행 파일과 플랫폼 적용 여부 판정.

### Changed (v3.46.2)

- 검사하지 않은 것과 검사해서 깨끗한 것을 리포트에서 구분한다. 71/71 테스트 통과.

## [3.46.1] - 2026-09-20

**같은 bash 3.2 결함이 `cross-verify.sh` 에도 있었다 — v3.46.0 은 한 파일만 고쳤다.** `git-sync-all.sh` 만 보고 "고쳤다" 고 판단했고, 전수 검사를 하지 않았다. mac-001 검증 에이전트가 짚었다: *"scripts/cross-verify.sh:70 은 설치된 캐시에서도, 고쳐진 상위 코드에서도 여전히 미수정."*

`bash -n` 문법 검사로는 안 잡힌다 — `declare -A` 는 bash 3.2 에서 **문법은 유효하고 실행할 때 실패한다.** 두 macOS 머신에서 설치본 문법 검사를 돌려 전부 통과를 확인했는데도 버그는 그대로 있었다. 문법 통과는 동작의 증거가 아니다.

### Fixed (v3.46.1)

- `scripts/cross-verify.sh:70` — `declare -A PID RCF OUTF START` 를 인덱스 배열(`LANE_PID`·`LANE_RCF`·`LANE_OUTF`·`LANE_START`)로 교체. 레인 목록은 순서가 고정이고 세 루프가 같은 순서로 돌므로 인덱스만 태우면 연상배열이 필요 없다. bash 3.2 에서 `PID[$lane]` 의 문자열 첨자는 산술식으로 평가돼 `expression recursion level exceeded` 로 터지고 레인 관리가 통째로 무너진다 — 2중 레인 교차검증이 macOS 에서 조용히 못 돌던 이유다.
- 전 스크립트 전수 검사 완료: `declare -A` · `mapfile` · `readarray` · `${v,,}` · `${v^^}` 잔존 0건(주석 제외).

## [3.46.0] - 2026-09-20

**v3.45.0 은 고쳤다고 말한 것을 담고 있지 않았다.** bash 3.2 수정과 시크릿 게이트는 v3.45.0 태그(`1930b43`) **다음** 커밋(`33e64c5`)에 들어갔는데 버전을 올리지도 태그를 달지도 않았다. 그래서 5대에 배포된 3.45.0 캐시에는 `declare -A` 가 그대로 있고 `_secret_hits` 는 없었다 — 함대 전체가 "고쳐진 버전" 이라는 이름의 안 고쳐진 코드를 받았다. 이 릴리스는 그 두 수정을 실제로 실어 나른다.

배포 대상 머신에서 sj-omen 적용 에이전트가 이 불일치를 잡아냈다: *"Lens 3.45.0 does NOT contain the bash 3.2 fix."* 설치 성공을 버전 문자열로 판정하면 이런 것이 통과한다 — 버전은 내용의 증거가 아니다.

### Fixed (v3.46.0)

- **macOS 기본 bash 3.2 에서 `/cs` 가 레포를 0개로 세던 결함** (내용은 v3.45.0 이후 커밋에 있었으나 릴리스되지 않았다). `declare -A` 가 3.2 에서 실패하고, 이어지는 문자열 첨자가 산술식으로 평가돼 `expression recursion level exceeded` 로 항목마다 터진다. 결과는 **REPOS 0개** — 아무것도 동기화하지 않고 종료코드 0 으로 "성공" 을 보고한다. 실측: macmini·mac-001 둘 다 `/bin/bash` 3.2.57 이고 **mac-001 에는 새 bash 가 아예 없다**. 개행 구분 문자열 + `case` 로 교체했고 macmini 의 실제 3.2 에서 직접 통과를 확인했다.
- **커밋 전 시크릿 게이트** (같은 사유로 미릴리스였다). `git add -A` 직전에 `git status` 를 훑어 `.env`·`*.bak*`·`*.pem`·`*.key`·`*.mirror`·`secret`·`credential` 후보가 있으면 **커밋하지 않고 보고만** 한다. 원격 머신을 대신 커밋해 줄 때 그 머신의 `.gitignore` 상태를 알 수 없고, 잘못 올라간 시크릿은 되돌릴 수 없다. 실측 4대에서 `.env.mirror` · `.env.bak-YYYYMMDD` 같은 변형 이름이 기존 패턴을 빠져나가 있었다. 끄기: `LENS_SYNC_SECRET_GUARD=0`, 패턴 교체: `LENS_SYNC_SECRET_RE`.

### Changed (v3.46.0)

- 릴리스 절차 교훈을 `CLAUDE.md` 가 아니라 여기 남긴다: **수정 커밋 뒤에 버전 범프가 따라오지 않으면 그 수정은 존재하지 않는 것과 같다.** 설치본 검증은 버전 문자열이 아니라 내용으로 해야 한다(`grep -c 'declare -A'` 같은 식으로).

## [3.45.0] - 2026-09-20

**`/cs` 가 매 런 같은 질문을 하게 만든 것은 판단력이 아니라 계약이었다.** 대표 지적: *"스크립트가 애매하게 짜져 있어서, 어떻게 처리할지 방향을 제대로 제시를 안 해주니 에이전트가 헷갈리니까 물어보겠지."* 정확했다.

v3.44 까지 `/cs` 는 매 런 `🌿 base 밖 원격 브랜치 (N)` 을 찍고, 스킬에는 **"report only — `/cs` 는 아무것도 지우지 않는다, 그건 `prune_branches.py` 와 사람의 몫이고 그 도구는 지우기 전에 묻는다"** 라고만 적혀 있었다. 문제를 보여주고, 손대는 것을 금지하고, 물어보는 도구를 가리킨 것이다. **에이전트에게 남은 합법 수단은 사용자에게 묻는 것 하나뿐이었다.** 질문은 설계였지 실수가 아니었다.

`sj-omen` 실측(2026-09-20): 19개 레포에 base 밖 브랜치 16개. 그중 **6개는 병합 증명됨**(모든 패치가 base 에 있음 — 잃을 것이 없다), **1개는 base 대비 0커밋**(`Returns_ERP_v20` 의 `origin/main`, base 는 `staging`). 16줄 중 7줄이 물어볼 이유가 없는 노이즈였고, 매 런 다시 물어봤다.

`lens.config.json` 에는 이미 `autoDeleteMergedBranch: true` 가 있었다. **`/cd` 만 그 키를 읽었고 `/cs` 는 읽지 않았다.** 이제 두 스킬에서 같은 뜻이다.

### Added (v3.45.0)

- **`/cs` 자동 정리(T7)** — 레포마다 push·회수 뒤, base 밖 목록을 모으기 **전에** `prune_branches.py --repo <repo> --remote <remote> --apply` 를 호출한다. 병합 증명은 재구현하지 않는다(patch-id + merge-tree 2단 — 두 벌로 갈라지면 한쪽이 반드시 틀린다). 순서가 중요하다: `push --delete` 가 트래킹 ref 까지 지우므로 방금 정리한 브랜치가 같은 런의 목록에 "방치" 로 다시 나타나지 않는다.
- **`LENS_SYNC_PRUNE_MERGED`** 환경변수 — `0` 이면 정리를 끈다. 우선순위: 환경변수 > `lens.config.json` 의 `autoDeleteMergedBranch` > 기본 on.
- `--json` 에 **`pruned[]`** 필드 (additive — 기존 키·순서 불변).
- 테스트 **S13·S14** — 병합 증명 브랜치는 삭제되고 미머지 브랜치는 보존되는지, 스위치를 끄면 둘 다 남는지. 110/110 PASS.

### Changed (v3.45.0)

- `skills/cs/SKILL.md` **「Branches outside base」절 전면 개정** — 비-base 브랜치를 4개 부류(병합 증명 / 0커밋 / 미머지 / 판정 불가)로 나누고 **부류마다 이미 취해진 행동**을 명시했다. 맨 앞 줄이 계약이다: *"Nothing in this section is a question for the user."* 미머지 목록은 상태 표시이며, 거기서 "이 브랜치들 어떻게 할까요?" 를 묻는 것이 v3.45 가 없애는 바로 그 회귀다.
- 리포트 버킷 이름 — `🌿 base 밖 원격 브랜치 … 다른 머신에 도달하지 않습니다` → **`🌿 아직 base 에 없는 작업 … 정보 표시. 이번 런이 처리할 것은 없습니다`** + `🧹 병합 완료 브랜치 정리 (N)`. 정리가 꺼져 있거나 python 이 없으면 **조용히 넘어가지 않고** 그 사실을 한 줄로 밝힌다.
- 회수(T4) 표의 `Scope` 행 — "sync/ 밖은 prune_branches.py 와 사람 몫" 은 더 이상 사실이 아니다. 회수는 여전히 `sync/` 전용이고, 그 밖은 T7 이 **병합 증명된 것만** 지운다(머지는 어디서도 하지 않는다).

### Fixed (v3.45.0)

- **0커밋 ref 가 영구 노이즈로 남던 것** — base 대비 고유 커밋이 없는 원격 ref 는 목록에서 제외한다. 보호돼 삭제되지 않는 레포 기본 브랜치(`Returns_ERP_v20` 의 `origin/main`)가 0커밋·0파일인데도 매 런 "base 밖" 으로 찍혔다.
- **`prune_branches.py` 가 Windows 에서 PATH 의 `gh` shim 을 건너뛰던 것** — `subprocess` 의 CreateProcess 는 PATHEXT 를 보지 않고 확장자 없는 이름에 `.exe` 만 붙인다. 그래서 PATH 앞의 `gh.cmd`(scoop·npm shim, 테스트 스텁)를 지나쳐 뒤쪽 `gh.exe` 를 집었다 — "PATH 로 gh 를 갈아끼웠다" 는 전제가 조용히 깨지고 열린 PR 보호 검사가 의도하지 않은 대상으로 나간다. `shutil.which` 로 해석한다(못 찾으면 종전처럼 실패 → fail-closed).
- 테스트 gh 스텁이 **회수와 prune 에 서로 다른 PR 집합**을 보여주던 것 — `--jq` 호출에는 canned 목록을, `--json` 호출에는 빈 배열을 돌려줘서 열린 PR head 보호가 테스트 안에서만 무력해졌다(S9·S10). 이제 같은 목록에서 JSON 을 만들고 실패 종료코드도 함께 흉내 낸다.
...
빠져 있던 것이 드러났다. `/cp` 에는 조사 에이전트 모델 규칙이 **`deep` D1 의 "중간 티어 명시" 한 줄뿐**이었다. 그래서 ① 판단이 섞인 어려운 축(아키텍처 추적·비가역 영향·보안)도 sonnet 한 칸이었고 ② **기본 등급에는 규칙이 아예 없어** 조사가 커지면 아무 기준 없이 떴다(2026-09-15 에 모델 없이 2개가 떠서 훅 경고가 두 번 무시된 지점). 난이도 사다리는 `/cc` 에만 있었고 그 사다리는 "적용 범위: `/cc` claude 레인" 으로 못 박혀 있어 `/cp` 를 덮지 않았다.

### Added (v3.44.0)

- `/cp` **「조사 에이전트는 난이도로 배분한다 — fable 은 관제만」** 절 — 나열·수집 = `haiku` / 일반 조사 = `sonnet` / **판단이 섞인 축(아키텍처 추적 · 비가역 영향 · 보안·권한 경로 · 엣지·실패) = `opus`** / 조사에 `fable` 금지. 읽기 전용은 Codex 우선(정액 0원). **등급 무관** — 기본 등급에서 조사가 커질 때도 같은 표.
- 승인 화면 `🔧` 줄에 `조사 {haiku a/sonnet b/opus c · codex d}` — 배분이 눈에 보여야 지켜진다.

### Changed (v3.44.0)

- `/cp deep` D1 — "중간 티어 명시" → **축마다 모델을 따로 명시**(⑤ 엣지·실패, ⑥ 통합·파급은 opus).
- 계약 카드 12 — 계획은 TOP 이 쓰되 **조사는 fable 로 하지 않는다**는 한 줄 추가.
- `docs/rules/harness-rules.md` §4.1 — 위 규칙을 SoT 로 기록(TOP 은 관제 1 spawn 에만).
...
- **`/cc` 「무정지 실행」 절** — 승인 한 번 = 마지막 검증과 최종 보고까지 묻지 않고 진행. 진행은 2분 보고로 알린다(알림 ≠ 질문). **정지 3종**만 멈춘다: ① 되돌리기 어려운 행동 ② 돈·외부 발송 ③ 승인 범위를 넘어야 하는 경우. ①② 중 **「항상 멈추는 행동」**(운영·staging 배포, 머지=배포 레포의 머지, DB 변경·삭제, 대량 삭제, force push, 발송·외부 게시·유료 대량 호출)은 계획에 적혀 있어도 멈추고 — 대표가 승인하며 "배포까지 해" 처럼 묻지 말고 하라고 한 것만 예외 — 그 밖의 행동은 계획에 없을 때만 멈춘다. `/cp` 는 이 계획에서 멈출 단계를 승인 보고 "멈추는 곳" 과 핸드오프 `정지 지점` 에 미리 적는다. 멈추는 동안 무관한 서브태스크는 계속 돈다. 헤드리스에서 정지에 닿으면 묻지 않고 보고 후 종료.
- **`hooks/pre-tool-ask.js` 실행 중 가드** — 이 세션이 연 게이트 원장(미종료·24시간 이내, 원장 색인 포함)이 있는 동안 header 가 `실행 승인` · `정지:비가역` · `정지:외부영향` · `정지:범위변경` · `검증 확인` · `실행 종료` 가 아닌 `AskUserQuestion` 을 거부하고, 어떤 header 로 다시 부를지 · 해당 없으면 판단해 진행 · 취소된 실행이면 닫는 명령(`closeLedger` 정확한 호출)과 `LENS_ASK_GUARD=0` 을 알려 준다. 다른 세션·닫힌 원장·세션 id 없음은 통과(fail-open). 이름표만 보는 검사라는 한계를 주석·규칙에 명시. 테스트 8 → 22.
- **적대적 검토(3 렌즈 · 에이전트 3개) 반영** — 안전 high: 초안의 "계획에 적힌 비가역 단계는 멈추지 않는다" 가 계획서에 '배포' 한 단어만 있어도 운영 배포를 무확인으로 내보냈다 → 「항상 멈추는 행동」 목록. 커버리지 high: manual 검증 확인 질문이 허용 header 에 없어 Phase 6 이 두 훅 사이에 갇혔다 → `검증 확인`. 훅 medium: 직접 호출 `/cc` 가 승인 전에 원장을 만들어 취소 후에도 24시간 질문이 막혔다 → 직접 호출은 1.5 승인 뒤 원장 생성, 취소 시 `closeLedger`. 그 밖에 1.5 분해 차이·TOP 상한 초과·우회로의 위험 단계·헤드리스·핵심원칙 1·7 문구.
- `docs/rules/harness-rules.md` §4.11.

### Changed (v3.40.0)

- **`/cc` 5.0 경로 전환을 묻지 않는다** — 계획서 트리거와 맞으면 자동으로 우회로 전환 + 한 줄 보고, 예상 못한 막힘은 범위 안에서 Leader 가 판단하고 범위를 넘어야만 풀릴 때 `정지:범위변경`. 3회 재시도 후에도 자동 전환, 우회로도 실패하면 `실행 종료`.
- `/cc` 핵심원칙 3 "승인은 실행 전에 한 번 · 승인 뒤에는 정지 3종만" · 기본값 4(UI 변경은 보고 후 묻지 않고 진행) · 0.4 브랜치 질문 header `정지:비가역` · 1.5 승인 header `실행 승인` · 6.2 는 최종 보고 먼저 + `실행 종료` · Worker 계약에 정지 3종 · 핸드오프 `다시 물을 단계` → `정지 지점`.
- `/cp` 계약 카드 11 · 승인 보고 "다음" 줄 · "지금 실행" 선택지 설명 · 핸드오프 `정지 지점` · 승인 질문 header `실행 승인`.

### Deferred (v3.40.0)

- 승인 선택지 "자동 / 단계마다 확인", 인벤토리 항목 골라 실행(`--only`) — 근거가 외부 도구 사례뿐이라 대표가 필요하다고 할 때.
...
- **`scripts/delegate.sh` — 외부 정액 엔진 병렬 디스패처.** `cross-verify.sh` 의 형제. `--task ID:ENGINE:PROMPT_FILE` 를 붙인 만큼 Codex·Grok 이 **동시에** 돌고 벽시계는 합이 아니라 max(엔진). 출력은 `TASK`/`DISPATCH` 두 줄 종류뿐. 기존 `codex-review.sh`·`grok-review.sh` 의 `--mode prompt` 를 그대로 태워서 stdin-EOF·argv 상한 등 v3.34~3.36 이 산 교훈을 재사용한다. **rc 0 인데 산출물이 비면 `empty`** 로 따로 세고 ok 로 치지 않는다 — 아무 말도 안 한 위임을 "완료"로 닫으면 브리핑을 받기로 했던 Worker 가 빈손으로 출발한다. **`elapsed` 는 레인별로 찍는다** — 완료 시각을 서브셸 안에서 스탬프한다. 보고 루프에서 시계를 읽으면 `wait` 전체 이후라 빠른 엔진이 느린 엔진의 벽시계를 뒤집어쓴다(실측: 수정 전 둘 다 35초 → 후 grok 11초 / codex 13초). **같은 `id:engine` 중복은 파싱 시점에 거부**한다(같은 경로에 동시 기록 → 둘 다 ok 로 보고되며 한 답이 두 요청에 배달된다 — 교차검증 Codex 레인이 잡은 결함). 중복 검사는 **소문자로 접어서** 한다 — 충돌하는 것은 문자열이 아니라 출력 경로이고, Windows 에서 `A-grok.out` 과 `a-grok.out` 은 한 파일이다(교차검증 3차 Codex 지적). 테스트 `tests/test_delegate.sh` **22/22** (스텁 레인, 실엔진 호출 0).
- **`/cc` 「엔진 배분」 절 (3엔진).** 난이도로 모델을 고르기 전에 **엔진부터** 고른다. `claude` 레인 = 쓰기·Skill·MCP·세션 컨텍스트가 필요한 것만. 그 외 읽고 답하는 일은 Codex(깊은 추론)·Grok(넓고 빠른 조회)로. 판정 한 줄: **"Claude 도 할 수 있다"는 Claude 레인에 남길 이유가 아니다 — 같은 품질이면 정액이 이긴다.** 모호하면 Grok 부터(십수 초, 틀려도 손해 0).
- **`/cc` Phase 1.35 정찰 위임.** fan-out 전 정찰을 기본적으로 Grok 에 맡긴다. Leader 는 세션 모델(대개 최상위)이라 **레포를 읽는 가장 비싼 방법**이다. 산출물은 승인표의 `건드릴 파일`·`읽은 근거 문서` 칸과 Worker 프롬프트의 `관련 파일` 슬롯을 채운다 — v3.34 가 *"그 슬롯을 무엇으로 채울지 정의한 문장이 어디에도 없다"* 고 지적한 배관이 여기서 이어진다. **경로 존재는 Leader 가 직접 확인**한다(외부 산출물은 근거지 판정이 아니다).
- **`lib/plan-manager.js` `deriveTodoItems()`** — 실행 Todo 를 계획서에서 **결정론으로 파생**한다. 성공기준(최상위, QA 만 닫는다) + 📋 인벤토리 `포함` 행 **전건** + 🛠 Plan A `#### 단계`. Plan B 의 `#### 단계` 를 Plan A 로 착각하지 않고, 해설 불릿·미치환 `{placeholder}` 는 세지 않는다. `valid:false` 면 `problems` 가 회귀할 Phase(0/1/2.45)를 이름으로 지목한다. **헤딩 깊이에 의존하지 않는다** — `/cp` 마크다운 템플릿은 `### Plan A` + `#### 단계`, `generatePlanContent()` 는 `## Plan A` + `### Steps` 를 만든다. 상대 레벨로 자르고(`sliceByHeading`), 회귀 테스트는 **생성기를 직접 돌려** 파싱한다(ko/en/de). **`{자리표시자}` 는 세 원천 모두에서 버린다**(`isTemplateLeftover` 공유) — 종전엔 단계만 걸러서 템플릿뿐인 계획서가 건강한 숫자로 게이트를 통과했다. 표 파서 `readInventoryTable()` 은 커버리지 게이트와 공유(중복 파서 금지). **형제 단계 제목을 전부 모은다** — Plan A 가 `### Step 1`/`### Step 2` 로 나뉘어도 누락이 없다(첫 블록만 읽으면 목록이 조용히 짧아지는데 `valid:true` 였다). **체크박스 우선 규칙은 목록 덩어리 단위**로만 적용한다 — 블록 전체에 걸면 `#### Step 1`(체크박스) 옆의 `#### Step 2`(번호 목록)가 통째로 사라진다. **단계 추출 규칙은 들여쓰기 하나로 줄였다** — 체크박스보다 깊은 불릿만 설명으로 배제하고, 같은 깊이면 표기·빈 줄과 무관하게 전부 단계다(교차검증 6·7·8차가 같은 뿌리의 변형 3개를 연속으로 잡았다). 단계는 **제목 아래에만 있다고 가정하지 않는다** — 첫 하위 제목 앞 머리말의 체크박스도 걷는다(일반 불릿은 전제 문장이라 제외). 교차검증 6~9차가 이 계열 변형 4개를 연속으로 잡았고, 매번 **조용히 짧아진 목록이 `valid:true` 로 게이트를 통과**했다. 변형 6가지를 테스트로 고정. 최종적으로 규칙 하나로 일반화했다(`stepsInPlanA`) — **체크박스는 어디에 있든 단계 · 일반 불릿은 단계 제목 아래에서만 · 체크박스보다 깊은 불릿은 그 단계의 설명**. 헬퍼 3개를 지워 코드가 줄었고 기존 테스트는 하나도 안 고치고 통과했다. 교차검증 6~10차가 이 계열 변형 5개를 연속으로 잡았으며, 매번 **조용히 짧아진 목록이 `valid:true` 로 게이트를 통과**했다. 단계 섹션 범위는 **깊이로 추적**해 하위 제목에서 벗어나지 않고, **관대한 폴백은 제거**했다 — Codex·Grok 두 레인이 독립적으로 같은 지점을 짚었다: 폴백이 Plan B 트리거를 실행 단계로 승격하고 `valid:true` 를 냈다. 단계를 산문과 구별할 수 없는 계획서엔 기계가 읽을 단계가 없는 것이고, 그렇게 말하는 것이 이 게이트의 존재 이유다. 제목·단계 스캔은 **코드 펜스를 인식한다** — 계획서의 ```bash 예시 안 `# 주석` 이 h1 으로 읽혀 Plan A 를 조기에 자르고 이후 단계를 전부 삼켰다(예시 안 체크박스도 이제 단계로 세지 않는다). 펜스 판정은 **CommonMark 규칙**(닫는 펜스는 같은 문자·여는 길이 이상)이라, 네 겹 펜스 안의 세 겹 예시를 종료로 오인하지 않는다. 닫는 펜스는 **info string 을 가질 수 없다**는 규칙까지 넣었고, 목록 마커 `-`·`*`·`+` 셋을 모두 읽는다(`extractGoal`·`extractPlanBTriggers` 도 같이 맞춤 — 단계만 셋을 받으면 `+` 로 쓴 계획서가 "성공 기준 없음"으로 엉뚱하게 회귀한다). 제목은 **선행 공백 3칸까지** 허용한다(CommonMark) — 컬럼 0 하드 앵커는 들여쓴 ` ## Plan B` 에서 Plan A 범위를 못 닫아 **Plan B 의 조건부 작업이 기본 실행 목록에 섞였다**. 번호 태스크 리스트(`1. [ ] …`, 유효한 GFM)도 체크박스로 읽는다 — 못 읽으면 일반 불릿으로 떨어져 단계 제목 밖에서 버려졌다. 여러 줄 HTML 주석은 **줄 수를 보존한 채 통째로 걷어내고**(`/cp` 템플릿의 안내 주석이 여러 줄이라, 줄 단위 제거는 속살을 문서에 남긴다), 백틱 펜스의 info string 에 백틱이 있으면 코드블록으로 보지 않는다. 테스트 **74/74**.
- **`/cp` Phase 5.0 게이트 9 (실행 Todo 게이트)** + 승인 화면 `📝 실행 Todo: N개 (성공기준 g + 인벤토리 i + 단계 s)`. 커버리지 게이트가 *계획서 안*의 누락을 잡는다면 이 게이트는 **계획서와 실행 목록 사이**의 누락을 잡는다 — 인벤토리 14행이 Todo 3개로 줄던 지점이 정확히 거기였다.
- **`docs/rules/harness-rules.md` §4.9 — 엔진 배분 SoT.**
- **`hooks/pre-tool-task.test.js`** — 훅이 강제하는 `TOP_CAP` 을 harness-rules §4.1 과 `/cc` SKILL.md 표기에 핀으로 묶는다. 상한을 문서에서만 3→2 로 바꾸고 상수를 3 으로 둔 실수를 교차검증 4차가 잡았다 — *상한이 산문에만 있으면 지켜지지 않는다*는 이 훅의 존재 이유를 한 층 위에서 반복한 셈이라 테스트로 못 박았다.

### Changed (v3.38.0)

- **`/cp` Phase 4 를 산문에서 명령으로 바꿨다.** 종전 15줄은 *"성공 기준과 Plan A 체크리스트로 항목을 생성한다"* 였고 아무도 세지 않았다. 이제 `deriveTodoItems` 를 돌리고 그 출력이 그대로 TodoWrite 입력이다. **손으로 짜지 마라.**
- **`/cc` TOP(`fable`) 상한 3 → 2** (harness-rules §4.1 동시 개정). 근거는 예산이 아니라 **구성**이다: 읽기가 정액 엔진으로 빠지면 Claude 레인에는 쓰기와 판단만 남는다 — 한 실행에 TOP 이 세 자리나 필요하면 분해가 잘못된 것이다. TOP 을 쓴 역할은 **승인표에 사유 한 줄**.
- **`/cc` 승인표에 `엔진` 칼럼 + `엔진 배분`·`TOP n/2`·`정찰` 줄**, 최종 보고에 `엔진 배분: claude N / codex M / grok K — 위임 실패 J건` 한 줄. 눈에 보이는 산출물이 되어야 산문 지시가 이행된다(§6 판정 규칙).
- **`/cc` Phase 2**: 핸드오프면 인벤토리 `포함` 항목을 실행 레벨 Todo 로 **전건** 등록. 실행 Todo 가 커버리지 원장보다 짧으면 그 원장이 실행 단계에서 다시 새는 것이다.
- **`/cp` 계획서 🧰 실행 전략에 `엔진 배분` 항목 추가** — 계획서가 이미 어느 작업을 어느 엔진으로 보낼지 적는다.

### 감수한 trade-off (v3.38.0)

- **외부 레인은 읽기 전용이다. 쓰기 위임은 넣지 않았다.** Codex 가 코드를 쓰면 Codex 는 Phase 4.5 에서 **자기가 쓴 코드의 독립 리뷰어일 수 없고**, 3중 게이트가 조용히 2중이 된다 — 소유자가 *"그 규칙으로 클로드 못 잡은 버그를 아주 많이 잡았어"*(2026-08-22) 라고 한 그 게이트다. 게다가 외부 쓰기는 PreToolUse 게이트·agent tracker·게이트 원장 **어느 것도 관측하지 못한다**. 절감폭은 줄지만 절감은 읽기에서만 취한다.
- **모델 슬러그는 하드코딩하지 않는다.** Codex 레인은 `~/.codex/models_cache.json` priority 1 을 자동 선택한다 — 이 기계 실측으로 현재 `gpt-6-astra`. 이름을 박으면 리더보드가 움직일 때 조용히 강등된다.
