...
function main() {
  try {
    ...
    // 6. Output response
    const response = {
      systemMessage: `Lens v3.24.0 activated - ${skills.length} skills from ${[...new Set(skills.map(s => s.plugin))].length} plugins detected | Agent Dashboard + Plan System ready`,
      hookSpecificOutput: {
    ...
    const fallback = {
      systemMessage: 'Lens v3.24.0 activated (scan skipped)',
      hookSpecificOutput: {
        hookEventName: 'SessionStart',
        error: err.message,
...
function buildAdditionalContext({ skillTable, memorySummary, keywordTable, planSummary, auditNudge, skillCount, pluginCount, config: cfg }) {
  ...
  // Header
  ctx += `# Lens v3.24.0 - Session Startup\n\n`;
...
function buildFallbackContext() {
  return `# Lens v3.24.0 - Session Startup
