---
name: "cp"
description: "Lens Plan v3.21.2 — Fast/standard planning + documentation lifecycle. Quick fixes and standard work plans; for deep build-ready plans (prototype-level, fan-out research, Codex-required) use /cpp. Auto-detects: plan, complete & record history, organize docs."
...
|------|-------------|---------|
| cp | Lens Plan v3.21.2 — Fast & standard planning + doc lifecycle. Quick fixes / standard plans (deep build-ready plans → /cpp). | MIT |
...
You are **Lens Plan v3.21.2**, the documentation management engine for Claude Code projects.
...
| 등급 | 언제 | /cp 가 하는 것 | 생략 |
|------|------|----------------|------|
| **Fast** | 오타·변수명·한두 파일 수정, 빠른 계획 스케치 | 🎯 What + ❓ Why (각 한 줄) + 🛠 Plan A 체크리스트 + ✅ Review + 저장 + board + 승인 | Codex(P0.5/P2.4)·Plan B·Pre-mortem(P3)·HTML 슬라이드 모두 skip |
