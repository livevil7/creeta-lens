...
|------|------|----------------|------|
| **Fast** | 범위가 작고 선택지가 거의 없으며 **되돌리기 쉬움** (오타·변수명·한두 파일) | 🎯 What + ❓ Why + 📋 작업 인벤토리 + 🛠 Plan A 체크리스트 + ✅ Review + 저장 + board + 승인 | Codex(P0.5/P2.4)·Plan B·Pre-mortem(P3)·HTML 슬라이드 skip |
...
- **우선순위**: 이 표는 아래 각 Phase 의 "항상/필수" 규칙보다 **우선**한다. Fast 등급에서 생략으로 표시된 Phase(P0.5 Codex·P2.4 합성·Plan B·P2.6 HTML 슬라이드·P3 Pre-mortem)는 실행하지 않는다. board 와 md 는 모든 등급에서 생성. 단, 이 우선순위는 **Phase 생략**에만 적용된다 — 내용 완전성(🛠How 단계·태스크·✅Review 검증·🧰실행전략·💡시사점·⚠️주의점·🔀Side Effect)은 우선순위 밖이며 압축·생략 대상이 아니다(원칙 0). **📋 작업 인벤토리(Phase 2.45)와 Phase 5.0 커버리지 게이트도 등급 무관 필수** — 어떤 등급도 이 둘을 생략하지 않는다(v3.32).
...
> **0-A. 아래 템플릿은 "크기 예시"가 아니라 "섹션 계약"이다 (v3.32 — 원칙 0 의 집행 조항)**
> Phase 2.5 의 템플릿 블록은 **어떤 섹션이 있어야 하는지**를 정하지, **각 섹션에 몇 개를 쓸지**는 정하지 않는다.
> - **예시의 항목 수를 따라하지 마라.** 템플릿이 불릿 2개·표 1행을 보여주는 것은 표기 형식을 보이려는 것뿐이다. 리스크가 9개면 9행, 대안이 4개면 4개, 단계가 23개면 23개를 쓴다. **항목 수는 조사 결과가 정하지 예시가 정하지 않는다.**
> - **섹션 목록은 최소집합이지 상한이 아니다.** 이 작업이 요구하는 주제가 템플릿·부록표에 없으면 **새 `##` 섹션을 만들어 쓴다.** 양식에 없다는 이유로 내용을 버리거나 다른 섹션에 욱여넣지 마라 — 그게 "양식에 얽매여 빼먹는" 실패 모드다.
> - **짧은 것도 합격 기준이 아니다.** "분량은 합격 기준이 아니다"(Phase 2.5)는 *길다고 통과시키지 마라*는 뜻이지 *짧게 쓰라*는 뜻이 아니다. 양방향으로 읽어라.
> - 집행은 산문이 아니라 **📋 작업 인벤토리 + Phase 5.0 커버리지 게이트**가 한다(Phase 2.45). v3.21 에서 이 원칙을 산문으로만 두었고, 그래서 압축이 재발했다.
...
**등급 무관 필수** — fast 에서도 쓴다. 항목이 1건이면 1행짜리 표이고 비용은 한 줄이다. 오히려 fast 가 압축 압력이 가장 세서 빠뜨리기 쉽다.
...
**필수 본문 (8)** — 등급 무관 항상: ① 결정 요약(관리자용, 맨 앞·짧게) ② 범위와 **비목표** ③ **📋 작업 인벤토리**(v3.32) ④ 핵심 결정과 **검토한 대안** ⑤ 실행 순서와 **단계별 검증** ⑥ 주요 위험·중단 조건·롤백 ⑦ **실행을 막는 미결 질문** ⑧ **DO NOT CHANGE**
...
4. **Body-Adaptive** — 불필요한 의식(ceremonial) 섹션 금지. spine 외 구조는 **주제가 정한다.** 단 **"분량 줄이기"는 목표가 아니다** — 항목은 사용자 언어로 필요한 만큼 **전량** 적는다(누락 금지). 큰 작업이면 길어지는 게 정상. **줄일 것** = 코딩 용어 주저리·중복·억지 의식 섹션 / **줄이면 안 될 것** = 항목 전량·파일별 변경·검증. **주제가 요구하는 섹션이 템플릿·부록표에 없으면 만들어 쓴다** — Body-Adaptive 는 *섹션을 빼는* 권한만이 아니라 *더하는* 권한이다(v3.32).
...
- **커버리지 (v3.32)**: S2 6축 조사와 사용자 요청에서 나온 항목이 여기 태스크로 **전부** 내려왔는지 `## 📋 작업 인벤토리` 표에 대조 기록한다(Standard 의 Phase 2.45 와 같은 표·같은 게이트). deep 은 조사 폭이 가장 넓어 **누락 위험도 가장 크다** — S7 이 이 표를 코드로 검사한다.
...
**커버리지 검사 (v3.32 — 산문 자기점검이 아니라 실행 검사)**: 위 항목들은 전부 "있는가" 를 묻는다. 누락은 그렇게 안 잡힌다 — 아래를 실행한다.

```bash
node -e "const m=require('${CLAUDE_PLUGIN_ROOT}/lib/plan-manager.js');const fs=require('fs');const r=m.validatePlanCoverage(fs.readFileSync(process.argv[1],'utf-8'));console.log(JSON.stringify(r,null,1));process.exit(r.valid?0:1)" docs/tasks/{id}.md
```

`valid:false` → S5 회귀. 카운트(`인벤토리 N건 → 포함 M / 제외 K`)는 S8 승인 화면에 표시한다.
