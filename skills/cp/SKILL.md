...
8. **승인은 보고 → 질문.** 보고 텍스트(링크 · 목표 · 🙋 대표 결정 · 직접 할 일 · 리스크 · 다음 행동)를 먼저 쓰고 그 다음에 질문 한 번. **완성 보고·수정 보고 모두 첫 줄은 링크다.** 선택지는 결과 문장 셋: **지금 실행** / **고칠 곳 있음** / **계획만 보관**.
   - **artifact 레인이면 질문은 페이지 안에 싣는다 (v3.50)** — 승인 · 🙋 대표 결정 · Phase 0 모호성 질문 · deep D2 질문 전부를 페이지의 질문 블록(객관식·주관식)으로. 대표는 페이지에서 고르거나 적고 [답변 보내기] 한 번으로 답한다(Phase 4.5 「질문은 페이지 안에서」). 이때 질문창은 띄우지 않는다 — 같은 질문을 두 곳에 두면 어느 답이 정본인지 갈린다. 이 아티팩트의 watch 가 `auto-replies armed` 가 아닐 때만 질문창을 같이 띄운다.
