...
|------|-------------|---------|
| ci | Creeta Install v3.23.0 — per-user plugin manifest ↔ actual install-state synchronizer. Installs missing managed plugins, removes only explicitly-excluded ones (backup + per-item confirm), reports foreign plugins read-only. Self-protecting: never uninstalls Lens. | MIT |
