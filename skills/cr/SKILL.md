...
- **2분 진행보고 (공통 규칙 · 훅으로 강제)** — 병렬 수집이 2분 이상이면 침묵 금지. **2분 주기**로 ① **생존 확인 결과**(추측 금지 — 실제 확인 후. **확인 없이 "진행 중" 금지**) ② 끝난 채널/남은 채널(N/M) ③ **이미 걷힌 소스는 대기 중이라도 먼저 보고**. **"아직입니다"만 적는 보고는 위반.** 백그라운드 수집 중 120초를 넘기면 `PostToolUse` 훅(`hooks/post-tool-progress.js`)이 보고 요구를 주입한다 — 리마인더가 떴는데 무응답이면 명백한 위반. (SoT: `docs/rules/harness-rules.md` §4.4.)
