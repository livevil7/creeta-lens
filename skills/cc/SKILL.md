...
scope: {포함 항목 수} · 다시 물을 단계: {라이브·배포 등, 없으면 없음}
...
> **`/cp` 핸드오프에 `[APPROVED]` 블록이 있으면 승인표를 다시 띄우지 않는다 (v3.39).** 대표 지적: 한 계획에 두 번 답하고 그 사이에 정찰 대기·브랜치 질문까지 끼었다. 아래 표는 **보고로만** 한 번 쓰고 바로 Phase 2 로 간다. **분해가 승인된 계획과 다를 때만**(계획에 없는 파일·범위를 건드리는 서브태스크가 생김) 그 차이만 보고하고 질문 1개로 묻는다. 계획서 frontmatter 가 `status: approved` 인 문서를 페이로드 없이 받아도 같다.
...
➡️ 지금 실행하면: {무엇이 어떤 순서로, 어느 단계에서 다시 묻는지}
```

그 다음 질문 한 번. 선택지는 결과 문장:
1. **지금 실행** — {위 ➡️ 줄}
2. **고칠 곳 있음** — 분해·접근 방식을 바꾼다
3. **취소** — 아무것도 바꾸지 않고 멈춘다
