...
2. **장시간 작업 진행보고 (2분 규칙 — 공통, v3.25 강화)** — 백그라운드 작업(Task 에이전트·**Workflow**·codex·장시간 Bash)이 2분 이상이면 침묵 금지. **2분 주기**로 보고하되 매번 세 가지를 **전부** 포함한다: ① **생존 확인 결과** — 추측 금지, `TaskOutput(block=false)`·산출 디렉토리 mtime 으로 **실제 확인 후** 보고. **확인 없이 "진행 중"이라 말하지 않는다.** ② 끝난 것/남은 것(N/M). ③ 지금 낼 수 있는 **부분 산출물은 대기 중이라도 먼저 낸다.** **"아직입니다"만 적는 보고는 위반.** 유실·정지 감지 시 즉시 보고하고 **복구보다 폐기·재판단 우선 검토.** 사용자의 VS Code 확장에는 진행창이 없어 스스로 확인할 수단이 없다 — 보고 책임은 전적으로 스킬에 있다. **이 주기는 훅이 강제한다** — Lens 훅이 마지막 보고 후 경과시간을 추적해 2분 초과 시 보고를 요구하는 리마인더를 주입한다(산문 규칙만으로는 조용히 스킵된 실측 이력). 위반 판정: 2분 초과 무보고, 또는 3요소 중 하나라도 빠진 보고. (SoT: `docs/rules/harness-rules.md` §4.4. `/c`·`/cc`·`/cp`·`/ccp`·`/cr`·`/cs` 공통.)
...
3. **Monitor Agent**: 백그라운드에서 2분마다 진행 상황 보고. 모든 Worker 완료 시 자동 종료.
...
| Worker (Easy) | 경량 티어 (현재 haiku) | 반복·조회·기계적 작업(사고과정 없음) — 파일 읽기·검색·단순 수정 |
| Worker (Medium) | 중간 티어 (현재 sonnet) | 단순 개발 — 정형 편집·판단 최소 |
| Worker (Hard) | 상위 티어 (현재 opus) | 사고과정이 들어가는 개발·리뷰·설계 — 트레이드오프 판단 |
| Worker (Critical) | TOP (현재 fable) | 최고 난도 — 비가역·보안·아키텍처 핵심 |
| Monitor | haiku | 상태 확인만 — 상위 모델 품질 이득 0 |
| Supervisor | **위험도 판정** (기본 상위 티어 / 고위험이면 TOP) | 연쇄 승격 폐지(v3.25). 리뷰는 사고과정 작업 → 기본 상위 티어(현재 opus). 파괴적·비가역 변경, 파일 5개+/diff 300줄+, 보안·권한 경로일 때만 TOP |
...
## 보고 형식
진행 현황: 1/3 작업 완료 ← 2분 후
진행 현황: 2/3 작업 완료 ← 4분 후
...
Monitor Agent는 **2분 주기**로 진행 상황을 보고합니다:
