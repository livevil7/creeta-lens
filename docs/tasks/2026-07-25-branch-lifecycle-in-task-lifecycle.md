...
| D14 | prune 이 **fetch URL 로 판정하고 push URL 로 삭제** | remote 에 `pushurl`(또는 `pushInsteadOf`)이 걸려 있으면 판정·보호·lease 는 전부 fetch 레포 기준인데 삭제는 push 레포로 나간다. 같은 이름·같은 SHA 의 `feat/x` 를 가진 upstream/fork 구성에서 재현 — 수정 전 코드가 **fork 의 브랜치를 실제로 삭제**했고(SHA 가 같아 lease 도 통과) upstream tracking ref 까지 부수 삭제됐다 | 5번째 fail-closed 로 차단. `--delete-without-pr-check` 로도 안 열림(판정 근거 레포와 삭제 대상 레포는 플래그로 같아지지 않는다). 실측 27개 레포 중 split remote 0개 |
| D15 | board 의 In Review 조인이 **head 이름만 대조** | head 이름이 같기만 하면 **다른 base 로 향하는 PR** 이 카드에 달라붙는다. fixture 재현: `base: master` 인 task 에 `feat/stacked → develop` PR 이 In Review 로 붙었다 | base 자격 검사 추가. `base` 없는 옛 문서는 이름만으로 조인(39개 중 `base:` 보유 1개뿐 — 엄격하게 가면 전 코퍼스 In Review 가 조용히 빈다). 안 붙인 건수는 상단 바에 표시 |
| D16 | 같은 조인이 **gh 응답 순서에 따라 결과가 달라짐** | 지시 범위 밖에서 발견. `byBranch.set()` 마지막-쓰기-승리라, 캐시의 PR 순서만 뒤집자 옛 코드가 tie task 에 **잘못된 base 의 PR** 을 붙였다 | 결정적 선택(자격 통과 후보 중 최소 번호)으로 해소 — 순서 무관 |
| D17 | `/cp done` 의 PR 증거가 **MERGED 상태만 확인** | `baseRefName` 을 안 봐서 **stacked PR**(다른 작업 브랜치로 머지 + head 삭제)이 `prMerged:true` → `merged-deleted` 를 만든다. 그 변경이 계획의 base 에 없는데도 task 가 완료 아카이브된다 | base 자격 통과 MERGED 가 **정확히 1개**일 때만 증거. 0개면 사유와 함께 `unknown`, 2개 이상이면 브랜치 재사용으로 보고 fail-closed |
| D18 | `/cp` 가 **원격에 없는 base 를 frontmatter 에 기록** | `resolveBase` 는 이름만 해석한다. upstream 이 stale 이면(`docs` 레포 실측: 원격이 비어 있는데 이름은 `main`) 존재하지 않는 base 가 문서에 박히고 `/cc` 가 실행 진입에서 실패한다 | PLAN 흐름이 `preflight()` 의 base(원격 ref 실존 확인 포함)를 쓰도록 변경 |

| D19 | 삭제 lease 가 **브랜치 tip 에만** 걸려 base 역행을 못 본다 | 삭제 근거는 둘("tip 이 판정 그대로" + "패치가 base 에 있다")인데 하나만 검증했다. throwaway 원격 재현: base 를 머지 이전으로 force-push → stale 판정으로 `--apply` → `삭제 완료`, 이후 `git branch --contains <B>` **출력 없음**(커밋이 어느 원격 ref 에도 없음) | `--atomic` + base·branch **이중 lease**. base refspec 은 no-op 이지만 lease 가 reset 된 base 를 되미는 write 까지 막는다(실측). 로컬은 `update-ref -z --stdin` 의 `verify`+`delete` 한 트랜잭션 |
| D20 | `gh pr list` 에 timeout·stdin 차단 없음 | 300초 sleep 하는 가짜 `gh.exe` 로 재현 — 워크스페이스 전수 순회가 **무기한 정지**, 외부 kill 로만 종료. 같은 파일의 `ls-remote` 에는 20초 가드가 이미 있었다 | `GH_PR_LIST_TIMEOUT_SECONDS=20` + `stdin=DEVNULL`. `TimeoutExpired` 를 **기존 fail-closed 사유 1번**으로 합류(새 경로 없음) |
| D21 | 진행보고 훅이 **`Workflow` 를 무장하지 못함** | `run_in_background` 필드가 없고 반환에 agent 문구가 하나도 없어 agent 봉투로는 영영 성립 불가. 4개 세션 25건 실측 | Workflow 전용 봉투(마커+식별자). 권한 거부로 **못 뜬 런치 1건**은 정상 탈락 — 마커 단독 판정을 안 쓰는 이유 |
| D22 | **11라운드에 내가 넣은 봉투 판별이 실제 payload 에서 성립하지 않음** | 두 경로 다 실패. ① content-block 배열이면 `JSON.stringify` 가 개행을 `\`+`n` 으로 escape → `agentId` 앞이 **단어 문자** → `\b` 가 깨져 식별자 미검출 ② 구조화 결과에는 문장 자체가 없음(실측: 이 세션 async Agent 런치 **41건 전부** `{status:'async_launched', agentId, outputFile, …}`) | `responseText()` 가 배열을 **`.text` 로 펼치도록** 근본 수정 + 구조화 철자 추가. 두 훅 동시 적용(문서가 "단일 감시 지점"이라 명시) |
| D23 | `SessionStart` 가 진행보고 상태를 초기화하지 않음 | stop.js 를 "삭제→스탬프" 로 고친 부작용. 이전 세션 시계가 살아남아 새 세션 **첫 폴링에서 즉시 발화 + "대기 7201초째"** 오보(실측 재현) | `startup`/`resume`/`clear` 만 삭제하는 **허용목록**. `compact`·`fork` 는 대화 도중 발화하고 `fork` 는 에이전트가 띄울 수 있어 보존 — 거기서 지우면 stop.js 가 막은 회피가 재생된다 |

**D6·D7 이 이 작업의 성격을 말해준다** — 계획서에 "생성 자동화보다 삭제 자동화가 먼저"라고 적었는데, 실제로는 **판정 자체가 틀린 것**이 더 먼저였다. D1 은 첫 `--apply` 에서, D7 은 두 번째 커밋에서 각각 터진다.

## 🔁 Codex 교차 리뷰 11라운드 + 12라운드 수정 (실행 중 추가)

Codex 가 매 커밋을 독립 리뷰했고 **11차까지 매 라운드 P1 이 나왔다.** 라운드별 P1: **5 → 3 → 2 → 4 → 2 → 3 → 2 → 1 → 1 → 2 → 1**. 단조 감소가 아니었고, 지적의 성격이 표면(하드코딩·오탐)에서 **순서·재사용·충돌·경쟁 상태** 같은 의미론으로 옮겨갔다.

> **왜 여기서 멈췄나**: 사전 선언한 기준("11차까지 돌리고 그 시점 상태로 커밋·푸시")대로 11차를 마지막 **리뷰**로 두었다. 다만 11차가 낸 P1 1건 + P2 3건은 **범위가 확정된 수정**이지 새 탐색이 아니므로 12라운드로 전부 처리했고, **12차 리뷰는 돌리지 않았다.** 매 라운드 새 P1 이 나온다는 것 자체가 **이 코드 영역(파괴적 git 연산)에서는 리뷰를 무한히 돌려도 0 이 되지 않는다**는 신호다. 무인 상태로 라이브에 올리지 않는 것(`/lens-upgrade` 보류)이 그 사실에 대한 대응이다.

**12라운드가 드러낸 것 — 내 수정이 만든 결함 2건(D22·D23)**: 11라운드에 넣은 "구조화 봉투 요구"는 **실제 payload 에서 한 번도 성립하지 않는** 판별이었고(D22), stop.js 의 "삭제→스탬프" 전환은 세션 경계에 낡은 시계를 남겼다(D23). 둘 다 이 표의 패턴 1("수정이 다음 결함을 만든다")의 새 사례다. **차이는 이번엔 실측 payload 41건을 직접 확인해서 잡았다는 것** — 추론이 아니라 트랜스크립트 대조가 잡았다.

**패턴 3가지가 반복됐다:**

1. **수정이 다음 결함을 만든다.** 거짓 `done` 을 고치니 거짓 `error` 가 생겼고(`launched` 상태로 해소), `--limit 200` 을 넣으니 "정확히 200개면 조용히 잘림" 이 생겼고, `namane-cms` 를 `master` 로 고정하니 잔여 `main` 이 삭제 대상이 됐다. 파괴적 연산 코드의 전형이다.
2. **규칙 문서를 고치고 코드·스킬을 안 따라간다.** 5·6차 지적의 대부분이 이 유형이었다 — 규칙을 야심차게 쓸수록 간극이 새로 생긴다. SoT 를 고칠 때 소비자 전부를 같이 봐야 한다.
3. **문서가 계획 범위를 넘어선다.** 규칙 문서가 레포별 브랜치 메타데이터를 요구했는데 계획서는 "1 task = 1 레포" 를 전제로 뒀다. **코드를 확장하지 않고 문서를 좁혔다** — 스칼라 4필드에 구조를 넣으면 파싱이 깨지고 핸드오프·DONE·board 소비자가 전부 영향받는다.

**실제 파괴를 재현한 것 3건** (이론이 아니었다):
- 태그 이름 충돌 → `legacy/foo` 의 tip 이 **어디에도 보존되지 않은 채 삭제**됨
- 셸 인젝션 → `pwn;touch${IFS}INJECTED` 브랜치로 `INJECTED` 파일이 **실제 생성**됨
- `--remote upstream` 에서 그 remote 의 **default 브랜치가 삭제**됨

**Codex 가 아니라 실측에서 찾은 것 1건**: prune 출력의 `base 분포` 에 `claude/why-so-many-branches-apcojh` 가 섞여 있었다 — 열린 PR 의 head 에 체크아웃된 레포에서 그 브랜치가 base 로 판정되고 있었다. `/cp` 가 그대로 계획했으면 **PR 이 main 이 아니라 다른 PR 의 브랜치를 향했다.** 금지 접두사(§3)를 base 후보에서 제외해 해소.

## 진행상황
- **마지막 업데이트**: 2026-07-26
- **현재 경로**: Plan A (구현·문서·리뷰 11라운드 완료 — **PR 머지 대기**)
- **작업 브랜치**: `feat/branch-lifecycle` (master tip `5cbf645` 에서 분기 — 미오염 검증 기준점). `livevil-setting` 은 `docs/progress-report-2min`
- **재개 포인트**: PR #2(creeta-lens) · PR #3(livevil-setting) 머지 → 버전 bump → 태그 → `/lens-upgrade` → 설치본 실동작 검증

### 남은 것 (사람 판단 필요)

- **`/lens-upgrade` 보류** — 의도적. 11라운드 내내 매 라운드 P1 이 나온 코드를 무인 상태로 라이브에 올리면 문제가 생겨도 알아챌 사람이 없다. 사용자가 있을 때 실행한다. **그때까지 설치본 캐시는 옛 코드라 사용자의 실제 Lens 동작은 바뀌지 않았다.**
- **`backup/macmini-*` 2건 태그 아카이브** — 커밋 169개인데 고유 변경 4파일(§0.6). 무엇을 살릴지는 내용 판단이라 도구가 못 정한다
- **`requireTaskBranch` 를 언제 `true` 로 올릴 것인가** — 현재 `false`(경고만). 올리면 브랜치 확보 실패가 곧 실행 차단이 된다
- **`docs` 레포 원격이 비어 있음** — base ref 부재로 판정 대상에서 제외 중. 초기 push 를 하든 대상에서 영구 제외하든 결정 필요
- **성공기준 4·5(머지 확인 후 완료 기록 / 병합 브랜치 자동 삭제)** — 구현·문서·fixture 재현까지 끝났으나 **실사용 왕복 검증은 PR #2 머지 후**에만 가능하다(아직 열려 있다)
...
- **병렬 worker lock 을 `withFileLock` 이 아니라 구조로 구현** — 계획서(🧰 실행 전략·R3)는 `lib/hook-utils.js` 의 파일 lock 재사용을 적었으나, 실제 통합 주체가 **Leader 단일·직렬**이라 잠글 경쟁자가 없다. 파일 lock 대신 "Leader 만, 하나씩" 이라는 구조적 lock 으로 구현했다. `withFileLock` 자체는 그대로 존재한다(다른 용도)
- **worktree 통합 수단을 merge 가 아니라 패치 전송으로** — worker 는 커밋하지 않으므로(커밋 주체는 Phase 7.4 Leader 뿐) worktree 브랜치는 보통 **커밋 0개**이고, merge·cherry-pick 은 **아무것도 안 가져오면서 "성공"** 한다 — 같은 결함이 "통합했다" 는 거짓 신호와 함께 재발한다. 게다가 worktree 는 task base 가 아니라 `origin/<기본 브랜치>` 에서 갈라지므로(도구 스키마 실측), base 가 `staging` 인 레포에서 merge 하면 무관한 이력이 PR 에 딸려 들어간다
