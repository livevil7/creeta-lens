...
- A4 해제 알림: 계획은 "해제는 `additionalContext` 로 상태당 1회" 였으나 **조용한 해제**로 바꿨다 → 공식 계약상 Stop 의 `additionalContext` 는 대화를 한 번 더 잇는다(두 번 막은 뒤 세 번째 턴을 억지로 만든다). 또 A4 의 "사용자 화면 문구 0건" 은 정확하지 않다 → 공식 계약상 block `reason` 은 사용자에게 경고로 보인다. 그래서 reason 첫 줄을 내부 용어 없는 사람 말 한 줄로 두고, 명령·목록은 둘째 줄부터(미충족 조건 집합당 최대 2회 노출).
- agentmemory 검증 37의 "health 가 critical 아님": 채울 수 없다 → 서버의 critical 판정식은 `heapUsed/heapTotal > 95%` 인데, 한가할 때 V8 이 힙을 반납해 이 비율이 늘 90~97% 다(실측 heapUsed 398MB / 노드 상한 4,144MB). 실제 기준(요약 성공 · MCP 연결 · 훅 5초 안 · heapUsed 가 노드 상한의 10% 안팎)으로 판정한다. 서버 버전은 0.9.29 에서 세션 목록 조회가 180초 504 로 깨져 0.9.27 유지. 힙 상한 플래그는 기본(4,144MB)보다 낮아져 넣지 않았다.
