...
- **마지막 업데이트**: 2026-06-11
- **현재 경로**: ✅ 완료·배포 (commit 247dbc0, tag v3.15.0 push, 캐시 3.15.0 갱신 — cpp/ 확인)
- **재개 포인트**: 없음. 새 세션 재시작 시 인벤토리에 `/cpp` 등장. (이 task 는 `/cp done` 으로 history 아카이브 가능.)
- **남은 후속(선택)**: gh release 생성, done-sweep 의 cpp 마커 인식(v2), UI 렌더 목업(현재는 ASCII 와이어프레임만).
