...
**① `/ch` 운영 헬스체크 디스패처** (new-skill, impact **high**)
- 문제: "featured 롤링 돼?·won 차고 있어?·발행 현황?·대기열 정상?"이 세션 진입마다 반복 + 매번 즉석 SSH/sqlite를 절대경로까지 수기 조립.
- 핵심 설계: 체크를 **새로 정의하지 않는다**(런북과 경쟁하는 4번째 SoT = "런북 먼저" 룰 위반). 대신 사용자가 *이미 만든* SoT를 **발견·실행**하는 얇은 디스패처 — `docs/runbooks/*.md`의 §TL;DR 원라이너, `check-operational-status.sh`, rotate dry-run, won-import 로그를 찾아 실행 → PASS/FAIL/이상치 한 화면. 없으면 그때만 '런북 생성 권장'.

**② `/cx` 세션 핸드오프** (extend-existing, impact med)
- 문제: 여러 세션·머신 동시 운용 중 '옆 세션 뭐 하는지, 산출물 어디 있는지' 직접 중계, 세션 끊기면 재파악 10회+.
- **진짜 코드 갭(검증됨):** `lib/agent-tracker.js:165-168` `initSession()`이 docstring은 "Preserves previous session's completed agents"라면서 실제로는 `createDefaultDashboard()`로 **빈 대시보드를 저장(wipe)**. memory-store엔 '진행작업+산출물 경로'가 없음.
- 외과적 버전: initSession이 직전 핸드오프 보존 + Stop훅이 산출물 절대경로·1줄 next-step 캡처 + SessionStart가 노출. `/cx`는 그 위 얇은 reader. `/cp` resume/handoff 필드와 명시 dedup(비계획 ad-hoc만 커버).

### DROP — 3건 (근거 강함, 흥미로워 보여도 거부)
