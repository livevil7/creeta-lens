...
**왜 예외가 남아 있는가**: `pr-manual` 레포는 base 머지가 곧 배포라(§1.2) 직접 push 할 수 없는데, 계획 밖에서 생긴 dirty 변경은 어딘가에 담겨야 한다. 그것을 `feat/` 로 위장하면 계획된 작업 브랜치와 구분이 사라져 `/cp done` 의 브랜치 소유 판정이 오염되고, `/cc` 는 남의 브랜치 위에서 커밋 허가를 받는다. **이름을 분리하는 것이 옳다.** ⚠️ v3.25~v3.30 은 *모든* 레포가 이 경로를 탔다 — 근거였던 "PR-only fail-closed" 전제가 철회된 경위는 §8 이다.
