...
| 실행 진입 | `/cc` | base 를 fetch·최신화한 뒤 `checkout -b <문서의 branch>` — **`requireTaskBranch` 값과 무관하게 항상 만든다**(그 플래그는 "만드느냐" 가 아니라 **"확보에 실패했을 때 차단하느냐"** 만 통제한다). diverged·base 판정 불가면 **차단**(fail-closed). dirty 면 차단하되, **그 dirty 가 전부 이 task 의 계획 산출물(plan md)이면 예외** — `/cp` 는 계획 문서를 커밋하지 않고 넘기므로 정상 핸드오프는 항상 dirty 다. 그 외 dirty 는 목록을 보여주고 (중단 / stash / 명시 승인) 을 묻는다. 헤드리스는 fail-closed |
...
| 완료 처리 | `/cd` | ① PR 생성/확인 → ② 머지 판정(§5) → ③ **브랜치 정리를 먼저 한다**(lease 통과, 또는 `autoDeleteMergedBranch:false` 같은 정당한 skip) → ④ 그 다음 history 기록 + task 문서 삭제 → ⑤ **미머지면 history 로 내리지 않고 "In Review" 로 보고** |
| 워크스페이스 스윕 | `/cs` | task 브랜치를 **소유하지 않는다.** task 브랜치는 건너뛴다(재포장·reset 금지 — v3.31 부터 스크립트가 강제). 기본 경로는 base 미러(직push)이고, `sync/` 브랜치는 `pr-manual` 레포·레거시 PR 모드에서만 만든다. 자기가 남긴 `sync/` 잔여물은 다음 런이 회수한다(§3.2) |
...
- **1 task = 1 레포가 이 규칙의 전제다.** frontmatter 의 `repo`/`base`/`branch`/`pr` 은 스칼라 4필드이고, 레포별 구조를 담지 않는다. 하나의 task 가 여러 레포를 건드려야 한다면 **레포마다 task 문서를 나눈다.**
  - 다중 레포를 한 문서에 담는 것은 **이번 범위 밖**이다(계획서의 명시적 전제). 스칼라 필드에 구조를 넣으면 파싱이 `[object Object]` 같은 값을 만들고, 핸드오프·DONE 소비자가 전부 깨진다.
  ...
  - 레포별 메타데이터를 frontmatter 로 구조화하는 것은 후속 과제다. 필요해지면 `plan-manager` 파싱·`/cp` 핸드오프·DONE 판정을 **함께** 바꿔야 한다.
...
- **`/cs` 가 자동으로 만드는 브랜치**: `sync/` 만 쓴다. **단 v3.31 부터 기본 경로는 미러(base 직push)라 `sync/` 를 아예 만들지 않는다** — 레거시 PR 모드(`LENS_SYNC_PR=1` opt-in)와 `syncPolicy: "pr-manual"` 레포(현재 `Returns_ERP_v20` 하나)에서만 생성된다.

**왜 예외가 남아 있는가**: `pr-manual` 레포는 base 머지가 곧 배포라(§1.2) 직접 push 할 수 없는데, 계획 밖에서 생긴 dirty 변경은 어딘가에 담겨야 한다. 그것을 `feat/` 로 위장하면 계획된 작업 브랜치와 구분이 사라져 `/cd` 의 브랜치 소유 판정이 오염되고, `/cc` 는 남의 브랜치 위에서 커밋 허가를 받는다. **이름을 분리하는 것이 옳다.** ⚠️ v3.25~v3.30 은 *모든* 레포가 이 경로를 탔다 — 근거였던 "PR-only fail-closed" 전제가 철회된 경위는 §8 이다.

예외의 대가로 규율을 더 세게 건다:

- `sync/` 는 **머지(또는 PR 종결) 즉시 삭제**한다. 원격·로컬 둘 다. 재사용하지 않는다 — 실행마다 새로 만든다.
- **`/cs` 는 매 실행에서 자기 `sync/` 잔여물을 회수한다**(v3.31 reconcile). ① 열린 `sync/` PR 은 병합 + head 삭제 — `pr-manual` 레포는 보고만 하고 머지는 사람이 한다. ② PR 이 없는 원격 `sync/*` 는 **§5 의 2단 병합 증명을 통과한 것만** §7.1 의 atomic 이중 lease 로 삭제한다(base 쪽은 no-op refspec, 어느 쪽이든 원격이 진전됐으면 push 전체가 거부된다 — 재시도·force 금지, 다음 런이 재판정). 회수 대상은 `sync/` **뿐**이다. 다른 접두사는 계속 `prune_branches.py` 와 사람의 몫이다(소유권 분리). "생성은 자동, 회수는 수동" 비대칭이 잔여물 누적의 원인이었다 — 실측 2026-08-14: 열린 자동 PR 6건(13~28일 방치), 병합 후 남은 브랜치 14개.
...
- **미머지로 닫힌 `sync/` PR 은 브랜치가 남는다.** 실측: `livevil-research` PR #5(`sync/2026-07-25-215207`)는 CLOSED·미머지이고 base 에 없는 패치 1개를 여전히 갖고 있다. 이 경우 브랜치를 그냥 지우면 그 변경이 사라진다 → **자동 삭제 금지, 사람이 내용을 확인한 뒤 처리**(§5 와 같은 원칙). 위 reconcile 도 이 조항을 그대로 따른다 — **증명되지 않으면 보존**이 기본값이고, 보존된 브랜치는 `/cs` 리포트의 "base 밖 원격 브랜치" 에 나이와 함께 계속 노출된다.
...
>
> 왜 중요한가: 거짓 "머지됨" 이 ref 를 지우는 것과 대칭으로, **거짓 "미푸시" 는 이미 머지된 작업에 대해 `/cd` 이 "push 하고 PR 을 여시겠습니까" 를 제안하게 만든다.** 둘 다 만들지 않는다.
...
(v3.39: 이 자격 검사를 쓰던 board 의 In Review 조인은 보드 파이프라인과 함께 삭제됐다. **`base` 를 적은 적이 없다는 것은 불일치의 증거가 아니다** 는 원칙은 남는다.)
...
- **도구가 출력하는 명령의 동적 값은 셸 인용한다.** git 은 브랜치 이름에 `;`·`$()`·`&`·백틱을 허용한다. 인용하지 않으면 그런 이름의 브랜치가 안내에 그대로 보간되고, 사용자가 블록을 복사해 실행하는 순간 **임의 셸 코드가 돈다**(재현 확인). `/cd` Phase 4 처럼 같은 형태의 명령을 만드는 경로는 전부 해당된다.
...
- 이 규칙은 `scripts/prune_branches.py` 와 `/cd` 의 Phase 4 **양쪽에 동일하게** 적용된다. 한쪽만 원자적이면 다른 쪽이 사고 경로로 남는다.
...
- ⚠️ **체크아웃된 브랜치는 건너뛰는 게 아니라 먼저 벗어난다.** `update-ref -d` 는 `git branch -D` 와 달리 현재 체크아웃된 브랜치도 **말없이 지워 HEAD 를 깨뜨린다**(git 2.53 실측) — 그건 사실이므로 가드는 유지한다. 그런데 **건너뛰면 더 나쁜 일이 생긴다**: `/cc` 직후 `/cd` 은 **최빈 경로**이고 그때 task 브랜치는 당연히 체크아웃 상태다. 건너뛰면 이후 완료 기록(history 작성·task 삭제)이 **이미 머지됐고 원격 ref 도 지워진 그 브랜치 위에서** 커밋되어 **base 에 영영 도달하지 못한 채 고립된다.**
...
- **PR 상태 조회는 레포를 못 박는다** — `gh pr list --repo <OWNER/REPO>`. remote URL 에서 해석하고, **해석 불가면 조회 불가와 같은 fail-closed**. 이 규칙은 `/cd` 과 `scripts/prune_branches.py` 와 board 빌더 **세 곳 모두**에 적용된다. 고정하지 않으면 gh 가 제 기본 레포를 골라 **다른 레포의 PR 을 "성공" 으로 반환**하고, 동명 PR 이 거짓 증거가 된다(실측 재현).
...
- 관련 설정 키(`lens.config.json`): `baseBranch`(레포별 명시 맵) / `requireTaskBranch` / `autoDeleteMergedBranch` / `branchPrefixes` / `syncPolicy`(레포별 `/cs` 정책 — `"pr-manual"` 이면 미러 금지, §3.2).

---

## 8. 결정 이력 — `/cs` 의 "PR 보장" 전제는 철회됐다 (2026-08-14)

§3.2 의 `sync/` 예외는 세 번 근거가 바뀌었다. 같은 반전을 네 번째로 반복하지 않으려면 무엇이 왜 뒤집혔는지가 규칙과 같은 자리에 있어야 한다.

- **v3.25 (2026-07-20) — 도입**: `/cs` 를 PR-only 로 바꿔 base 직접 push 를 폐지했다. 근거는 "올린 것의 기록으로서 PR 을 보장한다"였고, 그 fail-closed 조항이 `sync/` 예외를 정당화했다. ⚠️ 당시 **사용자 원선택은 "PR + 자동 머지"** 였는데 *"auto-merge 면 PR 은 기록용 포장일 뿐"* 이라는 Codex 반박이 채택되어 머지가 잘려 나갔다.
- **v3.27 (2026-08-05) — 자동 머지 복원**: 그 반박 채택이 잘못이었음을 자인했다(CHANGELOG 원문). 근거는 실측 피해다 — 병합 전까지 변경이 로컬 워킹트리에도 다른 머신에도 없어 라이브 메모리가 두 번 사라졌고(2026-08-02 17개·08-04), PR 방치로 Mac Mini 가 26커밋 뒤처졌다. **"기록용 포장" 이 이 도구가 원한 것 그 자체였다.**
- **v3.31 (2026-08-14) — 전제 철회**: 대표 발언으로 요구사항 자체가 사라졌다 — *"병합을 하던 뭘 하던 그건 모르겠고, 깔끔하게 클라우드에 올려놓고 다른 컴에서 그대로 받아 작업."* PR 은 목적이 아니라 의식(儀式)이었고, 그 의식이 실패 지점마다 잔여물을 남겼다(실측: 열린 자동 PR 6건·병합 후 잔여 브랜치 14개, 회수한 런 0). 기본은 base 미러(직push)가 되고 `sync/` 는 **배포 게이트 레포(`pr-manual`)와 레거시 opt-in 에만** 남는다.
- **읽는 법**: 이 예외가 살아 있는 이유는 이제 **배포 게이트 하나뿐**이지 PR 자체의 가치가 아니다. 이 절을 다시 고치려는 사람은 "PR 이 목적인가, 기록인가, 부산물인가" 를 먼저 답하고 — 그 답이 §1.2(머지=배포)를 건드리는지부터 확인한다.
