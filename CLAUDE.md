...
- v3.51.0 feat: **`/cp`·`/cc` 에 Jev 레인** — 항목 N개를 선택지로 판별하는 일(짝 맞추기·분류)을 `scripts/jev.js` 로. 문턱 0.80 · `--one-to-one` 충돌 검사 내장. 세기·숫자·언어 판정·되돌릴 수 없는 자동 반영에는 쓰지 않는다(SnapHolo 실측). SoT: `docs/rules/harness-rules.md` §4.9.1.
...
- v3.37.0 feat: **`/cp` 가 계획서를 띄운다 + 계획은 TOP 티어(`fable`)가 쓴다.** 소유자 지적 *"저장했으니 승인해라 이렇게만 보고를 해 … 맨날 계획서 찾는다고 탐색기 찾고 뭐하고 아주 지겨워 죽겠어"* / *"기획할 때는 왠만하면 fable 5.1 을 쓰게 해"*(2026-09-04). **안 본 문서에 대한 승인은 승인이 아니다.** 산출물 3종은 v3.20 대부터 정확히 만들어졌고 "풀 경로로 링크하라" 산문도 v3.25 부터 있었다 — 진 이유는 강도가 아니라 **성질**이다. 파일 생성은 모델 출력에 흔적이 남지만(산문만으로 18개 레포 이행, §6) **브라우저를 여는 일은 흔적을 안 남긴다.** 처방: ① **`lib/report-viewer.js`+`scripts/show-report.js`** — html(없으면 md)을 OS 기본 프로그램으로 열고 `.lens/report-shown.json` 에 기록. `browser`·`artifact` 만 "봤다"로 치고 SSH·헤드리스는 `remote` 로 정직하게 남겨 **게이트를 통과시키지 않는다** ② **Phase 4.5 신설** — 위치가 핵심. Pre-mortem(P3)이 md 를 바꾸므로 2.6 산출물을 띄우면 화면과 승인 대상이 어긋난다. P3 이후·P5 직전에 갱신 후 띄운다 ③ **Phase 5.0 §7 표시 게이트 · §8 `planner_model` 게이트**, 5.1 승인 화면 고정 블록, P6 Modify 는 재표시(열어 둔 탭은 수정 전 문서다) ④ **계획 = 사다리의 Critical 칸** — 세션이 TOP 이면 세션 내부, 아니면 P1~2.5 를 `Agent(model:"fable")` 에 **컨텍스트 전량과 함께** 위임하고 frontmatter `planner_model` 에 기록. TOP 상한은 그대로(1 spawn) ⑤ `hooks/post-tool-plan-doc.js` 가 두 지시를 문서 작성 시점에 주입(충족되면 조용). 테스트 `lib/report-viewer.test.js` 20/20 · 회귀 105/105 · Windows 실화면 실측. ⚠️ **v3.37.1 fix**: `.md` 를 OS 에 그대로 넘기면 Windows 에서 `start` 가 **exit 0 을 내면서 아무것도 안 연다**(연결 프로그램 부재 실측) — 표시 게이트는 통과하는데 화면엔 아무것도 없었다. win32 확장자 연결 확인 + 덱 없으면 `.lens/preview/{id}.html` 로 감싸서 연다. 상세: `CHANGELOG.md` + `docs/rules/harness-rules.md` §4.1·§4.8.
...
| `/cp` | 계획 엔진 — **2등급(기본/deep) · 3종류(신규/개선/조사보고)** | 계약 카드 → 목표·왜·종류 + 조기 보고 → 조사 3축 + 외부 레인 → 인벤토리 → 문서(계약 섹션 + 주제가 요구하는 것) → Pre-mortem → Todo 파생·네이티브 등록 → **엔진 네이티브 화면으로 띄우기** → 게이트 → 보고 → 질문 1회. deep = 6축 조사 + Codex 하드게이트 + 빌드레디 태스크 |
...
| UserPromptSubmit | Every message | `scripts/user-prompt-handler.js` | **v3.48** — stamps this session's `lastContactAt` (the user just saw the screen). The `/command` OVERRIDE is gone: it read the wrong input field (`userMessage`, not `prompt`) so it never fired, and fixed it would have injected "Do NOT call AskUserQuestion". Output `{}` |
| PreToolUse | Before Task tool | `hooks/pre-tool-task.js` | Registers sub-agent as "running" in dashboard |
| PreToolUse | Before AskUserQuestion | `hooks/pre-tool-ask.js` | **v3.39** — denies a question dialog with no report written since the last tool result (<40 chars). **v3.40** — while this session's gate ledger is open (an approved `/cc` run), denies any question whose header is not `실행 승인`·`정지:비가역`·`정지:외부영향`·`정지:범위변경`·`검증 확인`·`실행 종료`. Fail-open. `LENS_ASK_GUARD=0` |
| PostToolUse | After Write/Edit | `hooks/post-tool-plan-doc.js` | Plan doc gates (structure·coverage·kind) as advisory context. v3.39: root from the file path, no approval hints once `status: approved`, identical message not repeated, machine-only absolute path warning |
...
| SubagentStop | A subagent finishes | `hooks/subagent-stop.js` | **v3.48** — resolves this session's `launched` dashboard entry by `agent_id` (the async envelope's `agentId`) → `done`. Oldest untagged launched of the same type as fallback |
| PostToolUseFailure | Task/Agent/Workflow failed | `hooks/post-tool-task.js --failed` | **v3.48** — marks the entry `error` by `tool_use_id` |
...
| Session Store | `session-store.js` | `bind()`, `filePath()`, `read()`, `write()`, `update()`, `isSubagentCall()`, `topTierDelegated()`, `prune()` | **v3.48** — 훅 런타임 상태(`progress`·`dashboard`·`blocks`)를 `~/.claude/lens/sessions/<session_id>/` 에. `session_id` 없으면 null → 호출부가 종전 레포 경로로 폴백. 테스트 `lib/session-store.test.js` |
| Spawn Envelope | `spawn-envelope.js` | `classify()`, `responseText()`, `isAgentLaunchEnvelope()`, `isWorkflowLaunchEnvelope()`, `workflowName()` | **v3.48** — Agent·Workflow 백그라운드 런치 판별 공용(post-tool-task·post-tool-progress). 3.47 은 두 훅이 각자 정규식을 들고 어긋나 Workflow 를 done 으로 적었다 |
