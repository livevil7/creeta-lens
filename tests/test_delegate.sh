#!/usr/bin/env bash
# Regression tests for the external-engine dispatcher (v3.38; Grok engine removed v3.41).
#
...
# stdin: `codex exec` appends piped stdin to the prompt and blocks forever under
# a harness pipe that never reaches EOF.
...
# The owner cancelled the Grok subscription (2026-09-15). A leftover `ID:grok:FILE`
# must be refused at parse time, not dispatched to a script that no longer exists.
check "grok 엔진은 거부한다 (v3.41 제거)" \
  "! bash '$DEL' --task \"a:grok:$TMP/p.txt\" --dir '$TMP/o' >/dev/null 2>&1"
check "없는 프롬프트 파일은 거부한다" \
  "! bash '$DEL' --task \"a:codex:$TMP/nope.txt\" --dir '$TMP/o' >/dev/null 2>&1"
# The id names an output file; an unconstrained one walks out of --dir.
check "경로 문자가 든 id 는 거부한다 (디렉토리 탈출)" \
  "! bash '$DEL' --task \"../esc:codex:$TMP/p.txt\" --dir '$TMP/o' >/dev/null 2>&1"
check "PROMPT_FILE 가 빠지면 거부한다" \
  "! bash '$DEL' --task 'a:codex' --dir '$TMP/o' >/dev/null 2>&1"
...
check "값 없는 플래그는 무한루프 대신 즉시 거절한다" \
  "! timeout 10 bash '$DEL' --task \"a:codex:$TMP/p.txt\" --timeout >/dev/null 2>&1"
...
check "같은 id:engine 을 두 번 주면 거부한다 (산출물 덮어쓰기)" \
  "! bash '$DEL' --task \"a:codex:$TMP/p.txt\" --task \"a:codex:$TMP/p.txt\" --dir '$TMP/o' >/dev/null 2>&1"
# Case-insensitively, because the collision is on the output path and Windows
# treats A-codex.out and a-codex.out as one file.
check "대소문자만 다른 id 도 거부한다 (Windows 파일시스템)" \
  "! bash '$DEL' --task \"A:codex:$TMP/p.txt\" --task \"a:codex:$TMP/p.txt\" --dir '$TMP/o' >/dev/null 2>&1"

echo "== 3. 디스패치 (스텁 엔진) =="

# Stub the helper next to a copy of the dispatcher so HERE resolves to it.
...
OUT="$(run --task "a:codex:$TMP/p.txt" --task "b:codex:$TMP/p.txt")"
check "두 태스크 동시 디스패치 → 둘 다 ok" \
  "[ \"\$(statusof \"\$OUT\" a)\" = ok ] && [ \"\$(statusof \"\$OUT\" b)\" = ok ]"
check "DISPATCH 집계가 맞는다" \
  "printf '%s' \"\$OUT\" | grep -q 'DISPATCH DONE ok=2 down=0'"
check "산출물이 --dir 에 id 별로 남는다" \
  "grep -q 'codex briefing' '$TMP/o/a-codex.out' && grep -q 'codex briefing' '$TMP/o/b-codex.out'"
...
stub codex 0 'round one'
run --task "b:codex:$TMP/p.txt" >/dev/null
stub codex 2 ''
OUT="$(run --task "b:codex:$TMP/p.txt")"
check "직전 실행 산출물이 다음 실행에 남지 않는다" \
  "[ \"\$(statusof \"\$OUT\" b)\" = unavailable ] && [ ! -s '$TMP/o/b-codex.out' ]"

# The remaining two need tasks that behave differently on one engine, so this
# stub decides per prompt file: DOWN fails, SLOW takes three seconds.
cat > "$TMP/codex-review.sh" <<'MIXED'
#!/usr/bin/env bash
out=""; pf=""
while [ $# -gt 0 ]; do
  case "$1" in --out) out="$2" ;; --prompt-file) pf="$2" ;; esac
  shift
done
case "$(cat "$pf")" in
  *DOWN*) exit 2 ;;
  *SLOW*) sleep 3; printf slow > "$out"; exit 0 ;;
  *)      printf fast > "$out"; exit 0 ;;
esac
MIXED
echo "DOWN" > "$TMP/down.txt"
echo "SLOW" > "$TMP/slow.txt"

# A failed task must not take the healthy one with it — partial delegation is
# still worth having, and /cc degrades per task.
OUT="$(run --task "a:codex:$TMP/p.txt" --task "b:codex:$TMP/down.txt")"
check "한 태스크가 죽어도 다른 태스크 결과는 살아 돌아온다" \
  "[ \"\$(statusof \"\$OUT\" a)\" = ok ] && [ \"\$(statusof \"\$OUT\" b)\" = unavailable ]"
check "디스패처는 태스크 실패를 종료 코드로 인코딩하지 않는다" \
  "run --task \"b:codex:$TMP/down.txt\" >/dev/null"

# elapsed was read in the report loop, which runs only after every task is
# waited on — so a fast task was filed with the slowest task's wall clock.
# These numbers are how a caller learns which task is worth waiting for.
OUT="$(run --task "a:codex:$TMP/p.txt" --task "b:codex:$TMP/slow.txt")"
elapsed_of() { printf '%s' "$1" | sed -n "s/^TASK $2 .*elapsed=\([0-9]*\)s .*/\1/p"; }
check "빠른 태스크가 느린 태스크의 벽시계를 뒤집어쓰지 않는다" \
